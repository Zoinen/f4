...
func (fm *frameManager) semanticWorkspaceTabs() map[string]any {
	...
	for i, screen := range fm.Screens {
		...
		// Export the typed surface identity in the shared tab model itself.
		// Header-only/incremental scene exports do not include serialized
		// screen frames, so deriving this only in an application adapter would
		// make those updates fall back to the panels icon.
		for frameIndex := len(screen.Frames) - 1; frameIndex >= 0; frameIndex-- {
			provider, ok := screen.Frames[frameIndex].(WorkspaceTabSurfaceProvider)
			if !ok {
				continue
			}
			if surfaceKind := strings.TrimSpace(provider.GetWorkspaceTabSurfaceKind()); surfaceKind != "" {
...
func (g *Group) HandleSemanticAction(action map[string]any) bool {
	...
	for index, child := range g.items {
		...
		if c, ok := child.(Container); ok {
			// A semantic action routed from Qt bypasses the normal mouse path at
			// every level of a nested dialog. Let nested focus containers update
			// their own focus index first, then select that container in this
