...
func (fm *frameManager) Run(reader *vtinput.Reader) {
	...
	// Terminal renderers need a heartbeat for cursor blinking and animations.
	// Native renderers can own that presentation detail and remain entirely
	// event-driven while the application is idle.
	wantsPeriodicRedraw := true
	if renderer, ok := fm.scr.Renderer.(PeriodicRedrawRenderer); ok {
		wantsPeriodicRedraw = renderer.WantsPeriodicRedraw()
	}
	if wantsPeriodicRedraw {
		go func() {
			for fm.running {
