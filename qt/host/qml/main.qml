...
ApplicationWindow {
    ...
    readonly property var workspaces: scene.screens || []
    ...
    function workspacePrimaryFrame(workspace) {
        var frames = workspace && workspace.frames ? workspace.frames : []
        var fallback = null
        for (var i = frames.length - 1; i >= 0; --i) {
            var frame = frames[i] || ({})
            var kind = cleanText(frame.kind).toLowerCase()
            if (kind === "panels" || kind === "shell"
                    || kind === "terminal" || kind === "editor"
                    || kind === "viewer")
                return frame
            if (!fallback && kind !== "desktop" && kind !== "menu"
    ...
    function workspaceIconSource(workspace) {
        var frame = workspacePrimaryFrame(workspace)
        var kind = frame ? cleanText(frame.kind).toLowerCase() : ""
        if (kind === "panels" || kind === "shell")
            return resolvedIconSource("folder-kanban", 16)
        if (kind === "terminal")
            return resolvedIconSource("square-terminal", 16)
        if (kind === "editor")
            return resolvedIconSource("file-code", 16)
        if (kind === "viewer")
            return resolvedIconSource("file-text", 16)
        return resolvedIconSource("file", 16)
    }

    function workspaceFolderName(path) {
        var value = cleanText(path).replace(/\\/g, "/")
    ...
    function workspaceDisplayTitle(workspace, fallbackIndex) {
        var frame = workspacePrimaryFrame(workspace)
        var kind = frame ? cleanText(frame.kind).toLowerCase() : ""
        if (frame && (kind === "panels" || kind === "shell")) {
            var panels = frame.panels || []
            var left = ""
            var right = ""
            for (var i = 0; i < panels.length; ++i) {
                var panel = panels[i] || ({})
                var folder = workspaceFolderName(panel.path || panel.title)
                if (Number(panel.side) === 0)
                    left = folder
                else if (Number(panel.side) === 1)
                    right = folder
            }
            if (left !== "" && right !== "")
                return left + " | " + right
        ...
        var title = frame ? cleanText(frame.title) : ""
        if (title === "")
            title = cleanText(workspace ? workspace.title : "")
        return title !== "" ? title : "Workspace " + (fallbackIndex + 1)
    }
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                readonly property bool current:
                                    ...
                                    || index === Number(root.scene.activeScreen)
                                readonly property string workspaceTitle:
                                    root.workspaceDisplayTitle(modelData, index)
                                readonly property string workspaceIcon:
                                    root.workspaceIconSource(modelData)
                                objectName: "workspaceTab" + index
                                width: Math.min(148,
                                                Math.max(72,
                                ...
                                IconLabel {
                                    id: workspaceTypeIcon
                                    anchors.left: parent.left
                                    anchors.leftMargin: 10
                                    anchors.verticalCenter: parent.verticalCenter
                                    icon.source: workspaceTab.workspaceIcon
                                    icon.width: 16
                                    icon.height: 16
                                ...
                                Text {
                                    id: workspaceLabel
                                    anchors.left: workspaceTypeIcon.right
                                    anchors.leftMargin: 8
                                    anchors.right: workspaceAttention.left
                                    anchors.rightMargin: 6
                                    anchors.verticalCenter: parent.verticalCenter
                                    text: workspaceTab.workspaceTitle
