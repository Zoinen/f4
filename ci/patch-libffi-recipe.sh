...
libffi_recipe_revision="$(
    conan list 'libffi/3.4.4:*' -r conancenter --format=json |
        "${python_command}" -c 'import json, sys; data = json.load(sys.stdin); revisions = data["conancenter"]["libffi/3.4.4"]["revisions"]; print(max(revisions, key=lambda revision: revisions[revision].get("timestamp", 0)))'
)"
conan download "libffi/3.4.4#${libffi_recipe_revision}" --only-recipe --remote=conancenter
libffi_recipe="$(conan cache path "libffi/3.4.4#${libffi_recipe_revision}")"
