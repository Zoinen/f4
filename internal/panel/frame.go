...
type PanelsFrame struct {
	...
	lastPTYGeometry         map[terminal.PtyBackend]PtyGeometry
...
func (pf *PanelsFrame) applyTerminalBusyChange(busy, localShell bool) bool {
	...
	if busy {
		...
		if ignoredPrompt {
			// A command can be entered before the initial prompt has finished
			// crossing the PTY. That prompt belongs to shell startup, not to the
			// command just sent.
...
func publishPanelCatalogImmediate(fp *FileSystemPanel, benchmark *navtrace.NavigationBenchmarkTrace) bool {
	...
	if benchmark != nil {
		modelStartedNs = navtrace.NavigationBenchmarkMonotonicNs()
	}
	...
	if benchmark != nil {
		modelBuiltNs := navtrace.NavigationBenchmarkMonotonicNs()
	...
	if side == activeSide && typedCompleteOK {
		...
		queued = typedCompleteRenderer.QueuePanelCatalogModelStateWithCommandLine(
			...
			navtrace.NavigationBenchmarkTraceName(benchmark))
		...
		queued = typedRenderer.QueuePanelCatalogModelState(side, model,
			shellTitle, navtrace.NavigationBenchmarkTraceName(benchmark))
	} else {
		mapStartedNs := int64(0)
		if benchmark != nil {
			mapStartedNs = navtrace.NavigationBenchmarkMonotonicNs()
		}
		panel := map[string]any(model.ToMap())
		if benchmark != nil {
			mappedNs := navtrace.NavigationBenchmarkMonotonicNs()
...
// SameFolderHistoryPath is the panel-facing compatibility name used by the
// upstream history bridge. The identity rules live in fileops so panel and
// file-operation navigation cannot disagree about URI or local paths.
func SameFolderHistoryPath(a, b string) bool {
	return fileops.SameFolderHistoryPath(a, b)
}
...
func (pf *PanelsFrame) UpdateMenuCheckmarks() {
	...
	// clearly have the legacy ten-row shape.
	if len(pf.MenuBar.Items) >= 5 && len(pf.MenuBar.Items[0].SubItems) >= 10 &&
		len(pf.MenuBar.Items[4].SubItems) >= 10 &&
		pf.MenuBar.Items[0].SubItems[0].Command == 0 &&
		pf.MenuBar.Items[4].SubItems[0].Command == 0 {
		legacyLeft, legacyRight := ViewModeMedium, ViewModeMedium
		leftSort, rightSort := SortName, SortName
		leftGroups, rightGroups := false, false
		if fsp, ok := pf.Panels[0].(*FileSystemPanel); ok {
			legacyLeft, leftSort, leftGroups = fsp.ViewMode, fsp.SortMode, fsp.UseSortGroups
		}
		if fsp, ok := pf.Panels[1].(*FileSystemPanel); ok {
			legacyRight, rightSort, rightGroups = fsp.ViewMode, fsp.SortMode, fsp.UseSortGroups
		}
		if pf.Wide && pf.WidePanel == 0 {
			legacyLeft = ViewModeWide
		}
		if pf.Wide && pf.WidePanel == 1 {
			legacyRight = ViewModeWide
		}
		modes := []struct {
			mode ViewMode
			key  string
		...
		for i, mode := range modes {
			pf.MenuBar.Items[0].SubItems[i].Text = getMenuText(legacyLeft, mode.mode, "&"+i18n.Msg("Menu.Left."+mode.key))
			pf.MenuBar.Items[4].SubItems[i].Text = getMenuText(legacyRight, mode.mode, "&"+i18n.Msg("Menu.Left."+mode.key))
		}
		sorts := []struct {
			mode SortMode
			key  string
		}{{SortName, "SortName"}, {SortExt, "SortExt"}, {SortTime, "SortTime"}, {SortSize, "SortSize"}, {SortUnsorted, "SortUnsorted"}}
		for i, mode := range sorts {
			pf.MenuBar.Items[0].SubItems[i+5].Text = getSortMenuText(leftSort, mode.mode, "&"+i18n.Msg("Menu."+mode.key))
			pf.MenuBar.Items[4].SubItems[i+5].Text = getSortMenuText(rightSort, mode.mode, "&"+i18n.Msg("Menu."+mode.key))
		}
		if len(pf.MenuBar.Items[0].SubItems) > 10 && len(pf.MenuBar.Items[4].SubItems) > 10 {
			label := "&" + i18n.Msg("Menu.SortUseGroups")
			pf.MenuBar.Items[0].SubItems[10].Text = getToggleMenuText(leftGroups, label)
			pf.MenuBar.Items[4].SubItems[10].Text = getToggleMenuText(rightGroups, label)
		}
	}
...
// consumeLocalOutput contains the complete routing performed for one local
// PTY read. Keeping it separate makes shell-mark/reflow behavior directly
...
func (pf *PanelsFrame) displayLocalOutput(shouldProcess bool, data []byte) {
	...
	pf.redrawAfterTerminalOutput(len(data), before != after)
}
...
func captureTerminalOutputSemanticState(term *terminal.TerminalView) terminalOutputSemanticState {
	if term == nil {
...
func (pf *PanelsFrame) redrawAfterTerminalOutput(byteCount int, semanticStateChanged bool) {
	...
	if navtrace.NavigationBenchmarkIsEnabled() {
		marker := navtrace.NavigationBenchmarkRenderMarker()
		traceID := ""
		...
		if marker != nil && marker.Trace != nil {
			...
			fields = append(fields, navtrace.NavigationBenchmarkMarkerFields(marker)...)
		}
		navtrace.NavigationBenchmarkEmit(traceID, "terminal.output.redraw", "go.pty", fields...)
	}
}
...
func (pf *PanelsFrame) ResizeConsole(w, h int) {
	...
	if pf.ShellMode == terminal.ShellModeHost {
		...
		// the command line and KeyBar never paint over child output.
		...
		// Ctrl+O keeps the command line and KeyBar. Only an alternate-screen
		// application owns the complete window.
		...
		if pty := pf.localPTY(); pty != nil && pf.terminalWorkspaceActive() {
			...
			pf.negotiatePTYGeometry(pty, w, termH, cw, ch)
			for _, remotePty := range pf.RemotePtys {
...
func (pf *PanelsFrame) ProcessKey(e *vtinput.InputEvent) bool {
	...
	if e.VirtualKeyCode == vtinput.VK_RETURN {
		...
		if commandInputActive && !pf.CmdLine.IsEmpty() {
			...
			// The local Unix PTY is persistent. Do not re-impose the panel path
			...
			previousIgnoreNextPrompt := pf.ignoreNextPrompt
			...
			if err != nil {
				...
				if trackedCommand {
					...
					pf.ignoreNextPrompt = previousIgnoreNextPrompt
				}
	...
	if e.Type == vtinput.KeyEventType && e.KeyDown &&
		...
		if pf.ShowPanels && (!pf.SearchFirstMode() || !pf.CommandLineFocused) &&
			(pf.Wide || (pf.ActiveIdx == 0 && pf.ShowRightPanel) || (pf.ActiveIdx == 1 && pf.ShowLeftPanel)) {
...
func (pf *PanelsFrame) RunProgressTaskAfterContext(lifetime context.Context, delay time.Duration, title, startMsg string, forked bool, worker func(ctx context.Context, update func(msg string, percent int)) error, onComplete func(err error)) {
	...
	if lifetime != nil {
		stopLifetimeWatch = context.AfterFunc(lifetime, func() {
			...
			uiFrames.PostTaskWithRedrawDecision(func() bool {
				if !dialogShown || dlg.IsDone() {
					return false
				}
				dlg.Close()
...
func (pf *PanelsFrame) showDriveMenuAt(panelIdx, selectPos int) {
		...
		if set, err := LoadBookmarks(BookmarksFilePath()); err == nil {
			...
			for i := range set {
				...
				menu.AddItem(vtui.MenuItem{
					...
					Icon: sysinfo.DriveMenuIconBookmark,
	...
	vtui.FrameManager.PushMenu(&DriveMenuFrame{VMenu: menu, bottomHint: i18n.Msg("Drive.BottomHint")})
}
...
func (pf *PanelsFrame) SyncPassivePanel() bool {
	...
	if target == "" || fileops.SameFolderHistoryPath(target, dst.PersistentPath()) {
		return false
	}
...
func FolderHistoryStep(history []string, current string, pos, direction int) (int, string, bool) {
	...
	if pos < 0 || pos >= len(history) || !fileops.SameFolderHistoryPath(history[pos], current) {
		pos = -1
		for i, path := range history {
			if fileops.SameFolderHistoryPath(path, current) {
				pos = i
				break
			}
		}
...
func (pf *PanelsFrame) NavigateAvailableFolderHistory(fsp *FileSystemPanel, history []string, startPos, step int) bool {
	...
	for pos := startPos; pos >= 0 && pos < len(history); pos += step {
		...
		if fsp.ProviderOpenTask != nil && fileops.SameFolderHistoryPath(fsp.ProviderOpenTarget, path) {
