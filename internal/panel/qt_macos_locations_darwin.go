...
func macOSLocationsMenuItem(pf *PanelsFrame, panelIdx int) (vtui.MenuItem, bool) {
	return vtui.MenuItem{
		ID:   "macos-locations",
		Text: i18n.Msg("DiskMenu.MacOSLocations"),
		Icon: "folder",
...
func newMacOSLocationsMenu(pf *PanelsFrame, panelIdx int) *vtui.VMenu {
	menu := vtui.NewVMenu(i18n.Msg("MacOSLocations.Title"))
	...
	if len(rows) == 0 {
		menu.ReplaceItems([]vtui.MenuItem{{
			ID: "loading", Text: i18n.Msg("DiskMenu.MacOSLoading"),
...
func refreshMacOSLocationsMenu(
	...
	client := plughost.CurrentPlatformServices()
	if client == nil || !client.Available() {
		...
		items = append(items, macOSLocationsRetryItem(menu, state, pf, panelIdx,
			plughost.ErrPlatformServicesUnavailable))
	...
	go func() {
		...
		err := client.Request(ctx, "macos.locations", nil, func(response plughost.PlatformResponse) error {
...
func (pf *PanelsFrame) startAirDrop(fsp *FileSystemPanel) {
	...
	fileops.ExecuteFileOpWithResult(pf, source, destination, names, tempDir, false, 1, func(copyErr error) {
		if copyErr != nil {
...
func (pf *PanelsFrame) requestAirDrop(paths []string, cleanupDir string) {
	...
	if client == nil || !client.Available() {
		...
		vtui.ShowMessage(" AirDrop ", plughost.ErrPlatformServicesUnavailable.Error(), []string{"&Ok"})
		return
	}
	frames := vtui.FrameManager
	go func() {
		...
		if err != nil {
			var structured *plughost.PlatformError
