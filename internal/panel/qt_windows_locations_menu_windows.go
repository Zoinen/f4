...
func addWindowsLocationsDriveItem(pf *PanelsFrame, panelIdx int, menu *vtui.VMenu) bool {
	menu.AddItem(vtui.MenuItem{
		...
		Text: i18n.Msg("WindowsLocations.DriveItem"),
		Icon: sysinfo.DriveMenuIconWindows,
...
func newWindowsLocationsMenu(pf *PanelsFrame, panelIdx int, parent *vtui.VMenu) vtui.Frame {
	...
	menu := &windowsLocationsMenu{
		VMenu:    vtui.NewVMenu(" " + i18n.Msg("WindowsLocations.Title") + " "),
		pf:       pf,
		panelIdx: panelIdx,
		parent:   parent,
		client:   client,
	...
	for index, row := range rows {
		...
		if row.loading {
			items = append(items, vtui.MenuItem{
				...
				Text:     strings.Repeat("  ", row.depth) + i18n.Msg("DiskMenu.ShellLoading"),
				Disabled: true,
			})
			continue
		}
		...
		items = append(items, vtui.MenuItem{
			ID:       row.node.URI,
			Text:     prefix + fallbackIcon + " " + dialog.EscapeAmpersand(row.node.Name),
			Shortcut: shortcut,
		})
	}
	if len(items) == 0 {
		items = append(items, vtui.MenuItem{
			ID: "empty", Text: i18n.Msg("DiskMenu.ShellEmpty"), Disabled: true,
		})
...
func showGalleryIndexingRequired(pf *PanelsFrame) *vtui.Window {
	dialog := vtui.ShowMessageEx(
		" "+i18n.Msg("WindowsLocations.GalleryTitle")+" ",
		i18n.Msg("WindowsLocations.GalleryIndexingInstruction")+"\n\n"+
			i18n.Msg("WindowsLocations.GalleryIndexingContent"),
		[]string{
			i18n.Msg("WindowsLocations.GalleryOpenIndexingSettings"),
			i18n.Msg("WindowsLocations.GalleryCancel"),
		},
		vtui.MessageWarn,
	)
	dialog.OnResult = func(code int) {
		...
		vtui.RunAsync(func(task *vtui.TaskContext) {
			...
			task.RunOnUI(func() {
				vtui.ShowMessageEx(
					" "+i18n.Msg("WindowsLocations.GalleryTitle")+" ",
					fmt.Sprintf(i18n.Msg("WindowsLocations.GallerySettingsErrorFmt"), err),
					[]string{"&Ok"},
					vtui.MessageWarn,
				)
			})
...
func windowsLocationNavigationError(node winshell.Node, target string) error {
	if node.FileSystemPath != "" {
		...
		if !info.IsDir() {
			return errors.New(i18n.Msg("WindowsLocations.AccessNotFolder"))
		}
	}
	return errors.New(i18n.Msg("WindowsLocations.AccessUnavailable"))
}
...
func showWindowsLocationAccessError(node winshell.Node, target string, err error) *vtui.Window {
	...
	if title == "" {
		title = strings.TrimSpace(i18n.Msg("WindowsLocations.Title"))
	}
	return vtui.ShowMessageEx(
		" "+title+" ",
		fmt.Sprintf(i18n.Msg("WindowsLocations.AccessErrorFmt"), target, err),
...
func (m *windowsLocationsMenu) executeInternalDrop(target winshell.Node, move bool) {
	...
	start := func(destination vfs.VFS) {
		...
		go fileops.ExecuteFileOpAtIn(m.pf, source.Vfs, destination, source.Vfs.GetPath(), names, destination.GetPath(), move,
			config.App.DefaultFileOpMode, func() {
				vtui.FrameManager.PostTask(func() {
					_ = destination.Close()
					m.pf.RefreshAll()
					vtui.FrameManager.Redraw()
