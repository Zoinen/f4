...
func (q *QuickViewPanel) semanticHeaderRows(item *FileEntry, innerW int) []extui.TextRowModel {
	...
	lines := []string{
		" " + item.Name,
		fmt.Sprintf(" %s: %s", i18n.Msg("QuickView.Size"), formatBytes(uint64(item.Size))),
	}
	if q.cacheReadErr != nil {
		lines = append(lines, "", " "+i18n.Msg("QuickView.ReadError")+": "+q.cacheReadErr.Error())
		return quickViewRows(lines)
	}
	status := ""
	switch {
	case q.cacheLoading:
		status = i18n.Msg("QuickView.Loading")
	case q.cacheLabel != "":
		status = q.cacheLabel
	case q.cacheImage:
		status = i18n.Msg("QuickView.Image")
	case q.cacheBinary:
		status = i18n.Msg("QuickView.Binary")
	}
...
func (q *QuickViewPanel) semanticWindowForLines(lines []string, viewportRows int) semantic.SemanticSurfaceWindow {
...
func (q *QuickViewPanel) HandleSemanticAction(action map[string]any) bool {
	if q == nil || semantic.String(action["target"]) != vtui.SemanticID(q) ||
		semantic.String(action["action"]) != "quickView.scroll" {
		return false
	}
	q.prepareSelection()
	if key := semantic.String(action["contentKey"]); key == "" || key != q.semanticContentKey {
		return false
	}
	innerW := max(1, q.X2-q.X1-1)
	q.ensureDisplayLayout(innerW)
	...
	requested := semantic.Int(action["visualRow"])
...
func makeQuickViewSelectionKey(filesystem vfs.VFS, path string, item vfs.VFSItem) quickViewSelectionKey {
	return quickViewSelectionKey{
		source:   plughost.MediaSourceKey(filesystem, path),
