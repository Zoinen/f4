...
func TestPanelsFrame_NavigateToPath(t *testing.T) {
	...
	if got := lp.Vfs.GetPath(); !fileops.SameFolderHistoryPath(got, originalPath) {
...
func TestRegisteredDriveIconUsesPluginSpecificGlyphs(t *testing.T) {
	tests := []struct {
		...
		{name: "Android", want: sysinfo.DriveMenuIconAndroid},
		{name: " iOS ", want: sysinfo.DriveMenuIconIOS},
		{name: "AI", want: sysinfo.DriveMenuIconAI},
		{name: "CloudFox", want: sysinfo.DriveMenuIconCloud},
		{name: "NetFox", want: sysinfo.DriveMenuIconNetwork},
		{name: "Custom drive", want: sysinfo.DriveMenuIconPhysical},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := sysinfo.RegisteredDriveIcon(tc.name); got != tc.want {
				t.Fatalf("sysinfo.RegisteredDriveIcon(%q) = %q, want %q", tc.name, got, tc.want)
			}
		})
	}
}
