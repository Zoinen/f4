...
func TestWindowsLocationsDriveItemUsesWindowsLogo(t *testing.T) {
	...
	if got := menu.Items[0].Icon; got != sysinfo.DriveMenuIconWindows {
		t.Fatalf("Windows locations icon = %q, want %q", got, sysinfo.DriveMenuIconWindows)
	}
...
func TestWindowsLocationsDriveSubmenuUsesNativeCascadeAndWrapperActions(t *testing.T) {
	...
	if !root.ProcessKey(&vtinput.InputEvent{Type: vtinput.KeyEventType, KeyDown: true, VirtualKeyCode: vtinput.VK_RIGHT}) {
	...
	if root.Items[0].SubmenuFrame == nil {
		t.Fatal("drive entry lost its submenu factory")
	}
