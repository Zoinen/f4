...
func newQuickViewProviderFixture(t *testing.T, namesAndData map[string][]byte) (*QuickViewPanel, *FileSystemPanel, *vtui.ScreenBuf) {
	...
	pnl := &FileSystemPanel{Vfs: filesystem, Entries: entries}
	pnl.ScreenObject.SetPosition(0, 0, 39, 19)
	quickView := NewQuickViewPanel(pnl)
...
func TestQuickViewProviderSelectionChangeCancelsStaleResult(t *testing.T) {
	...
	registerQuickViewTestProvider(t, provider)
	quickView, pnl, screen := newQuickViewProviderFixture(t, map[string][]byte{
		"a.qvcancel": []byte("a"),
		"b.qvcancel": []byte("b"),
	})
	// Map iteration does not promise row order.
	for index, entry := range pnl.Entries {
...
func TestQuickViewProviderCacheKeyIncludesRevisionAndVFS(t *testing.T) {
	...
	registerQuickViewTestProvider(t, provider)
	quickView, pnl, screen := newQuickViewProviderFixture(t, map[string][]byte{"same.qvkey": []byte("one")})
	pnl.Entries[0].Revision = "revision-one"
	quickView.Show(screen)
	waitForQuickView(t, func() bool { return !quickView.cacheLoading })
	quickView.Show(screen)
	if calls.Load() != 1 {
	...
	pnl.Entries[0].Revision = "revision-two"
	quickView.Show(screen)
	waitForQuickView(t, func() bool { return !quickView.cacheLoading })
	if calls.Load() != 2 {
