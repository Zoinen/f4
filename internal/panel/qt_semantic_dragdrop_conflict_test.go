		...
		for _, move := range []bool{false, true} {
			for _, choice := range []string{"Abort", "Skip", "Retry then Abort"} {
				t.Run(fmt.Sprintf("%s/move=%v/%s", kind, move, choice), func(t *testing.T) {
					...
					fileops.ExecuteFileOpWithResult(rig.panels, source, destination, []string{name}, destination.GetPath(), move, 0, func(err error) { result <- err })
					dlg := waitForTransferDialog(t, " Error ").(*vtui.Window)
					...
					if choice == "Retry then Abort" {
						testutil.ClickDialogButton(t, dlg, "Retry")
					...
					testutil.ClickDialogButton(t, dlg, button)
					vtui.FrameManager.Pop()
				...
				for _, move := range []bool{false, true} {
					for _, choice := range []string{"Cancel", "Skip", "Overwrite"} {
						t.Run(fmt.Sprintf("directory=%v/%s-to-%s/move=%v/%s", directory, sourceKind, targetKind, move, choice), func(t *testing.T) {
							...
							result := make(chan error, 1)
							fileops.ExecuteFileOpAtWithResult(rig.panels, plan.source, plan.target.fs, plan.sourceDir, plan.names, plan.target.Dir, plan.move, 0, func(err error) { result <- err })
							dlg := waitForTransferDialog(t, i18n.Msg("Warning.Title")).(*vtui.Window)
							testutil.ClickDialogButton(t, dlg, choice)
							vtui.FrameManager.Pop()
							deadline := time.After(5 * time.Second)
							for {
								select {
