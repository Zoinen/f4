...
func newStableInfoTestPanel(x, y, w, h int, filesystem vfs.VFS, entries []*FileEntry) *FileSystemPanel {
	...
	// panel.NewFileSystemPanel starts its real directory load immediately. Provider
...
func TestInfoPanel_CopyUsesFullProviderValueWhenDisplayIsTruncated(t *testing.T) {
	...
	for i := range ip.rows {
		...
		// copyCurrent copies panel.InfoRow.value verbatim (covered separately by
...
// TestFormatBytes_TogglesWithConfig verifies panel.FormatBytes routes to
