		...
		for _, toArchive := range []bool{false, true} {
			...
			for _, move := range []bool{false, true} {
				t.Run(fmt.Sprintf("sourceArchive=%v/targetArchive=%v/move=%v", fromArchive, toArchive, move), func(t *testing.T) {
					...
					fileops.ExecuteFileOpAtWithResult(nil, plan.source, plan.target.fs, plan.sourceDir, plan.names, plan.target.Dir, plan.move, 0, func(err error) { result <- err })
...
func TestSemanticDropPlan(t *testing.T) {
	...
	t.Run("retry after cancelled copy refresh", func(t *testing.T) {
		...
		for _, action := range []map[string]any{a, folder} {
			...
			if semantic.String(action["entryId"]) != "" && (plan.target.entryIdx != 1 || plan.target.Dir != dst.Vfs.Join(dst.Vfs.GetPath(), "folder")) {
	...
	t.Run("prepare complete selection and reject stale response", func(t *testing.T) {
		...
		paths := semantic.StringSlice(response["paths"])
...
func TestQueuedDropCancelKeepsOriginatingWorkspace(t *testing.T) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		result := make(chan error, 1)
		fileops.ExecuteFileOpWithResult(rig.panels, vfs.NewOSVFS(srcDir), vfs.NewOSVFS(dstDir), []string{"conflict.txt"}, dstDir, false, 0, func(err error) { result <- err })
		dlg := waitForTransferDialog(t, i18n.Msg("Warning.Title")).(*vtui.Window)
...
func TestExternalDropCancelStopsRemainingGroups(t *testing.T) {
	...
	defer rig.panels.Close()
	oldMode := config.App.DefaultFileOpMode
	config.App.DefaultFileOpMode = 0
	defer func() { config.App.DefaultFileOpMode = oldMode }()
...
func TestSemanticDragPreparationAfterPublishedCatalog(t *testing.T) {
	for _, paged := range []bool{false, true} {
		t.Run(fmt.Sprint("paged=", paged), func(t *testing.T) {
			oldRows := semantic.SetPanelCatalogRowsEnabled(paged)
			oldMetadata := semantic.SetPanelCatalogMetadataEnabled(false)
			defer semantic.SetPanelCatalogRowsEnabled(oldRows)
			defer semantic.SetPanelCatalogMetadataEnabled(oldMetadata)
