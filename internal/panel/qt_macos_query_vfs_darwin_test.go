...
func TestMacOSQueryVFSStreamsDuplicateNamesAndDelegatesLocalOperations(t *testing.T) {
	...
	client := testMacOSPlatformServices(func(ctx context.Context, operation string, payload map[string]any, respond func(plughost.PlatformResponse) error) error {
		if operation != "macos.query" {
			t.Errorf("operation = %q", operation)
		}
		if err := respond(plughost.PlatformResponse{Operation: operation, Chunk: true,
		...
		return respond(plughost.PlatformResponse{Operation: operation, Final: true,
			Payload: map[string]any{"items": []any{map[string]any{
...
func TestMacOSQueryVFSSpotlightTimestampDriftStillFeedsGalleryBroker(t *testing.T) {
	...
	broker, err := plughost.NewExtUiMediaBroker()
	if err != nil {
		t.Fatal(err)
	}
	defer broker.Close()
	descriptor := broker.Register(plughost.MediaSourceRegistration{
...
func TestMacOSQueryVFSWatchUsesPlatformEventsAndCancels(t *testing.T) {
	...
	client := testMacOSPlatformServices(func(ctx context.Context, operation string, payload map[string]any, respond func(plughost.PlatformResponse) error) error {
		if operation != "macos.watch" {
			t.Errorf("operation = %q", operation)
		}
		if err := respond(plughost.PlatformResponse{Operation: operation, Chunk: true, Payload: map[string]any{"ready": true}}); err != nil {
			return err
		}
		if err := respond(plughost.PlatformResponse{Operation: operation, Event: true, Payload: map[string]any{"refresh": true}}); err != nil {
			return err
		}
		<-ctx.Done()
		cancelSeen <- struct{}{}
...
func TestMacOSNetworkResultResolvesThroughNativeMountPayload(t *testing.T) {
	...
	client := testMacOSPlatformServices(func(ctx context.Context, operation string, payload map[string]any, respond func(plughost.PlatformResponse) error) error {
		requestSeen <- map[string]any{"operation": operation, "payload": payload}
		return respond(plughost.PlatformResponse{Operation: operation, Final: true,
	...
	case request := <-requestSeen:
		if platformAnyString(request["operation"]) != "macos.mount" {
			t.Fatalf("operation = %q", platformAnyString(request["operation"]))
		}
...
// The panel tests exercise the native service contract. Transport framing and
// cancellation messages are tested independently in plughost.
type testMacOSPlatformServices func(context.Context, string, map[string]any, func(plughost.PlatformResponse) error) error

func (testMacOSPlatformServices) Available() bool { return true }
func (f testMacOSPlatformServices) Request(ctx context.Context, operation string, payload map[string]any, respond func(plughost.PlatformResponse) error) error {
	return f(ctx, operation, payload, respond)
}
