...
func extUiSendMessageWithBenchmark(w io.Writer, msg map[string]any, benchmark *navtrace.NavigationBenchmarkMessage) error {
	if benchmark != nil {
		navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.marshal.begin", "go.transport",
	...
	if err != nil {
		if benchmark != nil {
			navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.marshal.end", "go.transport",
				"phase", benchmark.Phase, "phaseSequence", benchmark.PhaseSequence,
				"sceneSequence", benchmark.SceneSequence, "messageType", benchmark.MessageType,
	...
	if benchmark != nil {
		navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.header_write.begin", "go.transport",
			"phase", benchmark.Phase, "phaseSequence", benchmark.PhaseSequence,
			"sceneSequence", benchmark.SceneSequence, "messageType", benchmark.MessageType,
			"bytes", len(hdr))
	}
	if _, err := w.Write(hdr[:]); err != nil {
		if benchmark != nil {
			navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.header_write.end", "go.transport",
				"phase", benchmark.Phase, "phaseSequence", benchmark.PhaseSequence,
				"sceneSequence", benchmark.SceneSequence, "messageType", benchmark.MessageType,
				"ok", false, "error", err.Error())
		}
	...
	if benchmark != nil {
		...
		navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.payload_write.begin", "go.transport",
			"phase", benchmark.Phase, "phaseSequence", benchmark.PhaseSequence,
			"sceneSequence", benchmark.SceneSequence, "messageType", benchmark.MessageType,
	...
	if benchmark != nil {
		...
		navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.payload_write.end", "go.transport", fields...)
	}
	return err
}
...
func (s *extUiMessageSender) Send(msg map[string]any) error {
	return s.SendWithBenchmark(msg, navtrace.NavigationBenchmarkMessageFromMap(msg))
}

func (s *extUiMessageSender) SendWithBenchmark(msg map[string]any, benchmark *navtrace.NavigationBenchmarkMessage) error {
...
func (s *extUiMessageSender) SendSemanticSnapshot(streamID string,
	...
	return s.sendWithBenchmark(payload,
		navtrace.NavigationBenchmarkMessageFromMap(payload), streamID,
		extui.KindSnapshot, true)
}

func (s *extUiMessageSender) sendWithBenchmark(msg map[string]any,
	benchmark *navtrace.NavigationBenchmarkMessage, streamID, kind string,
	semantic bool,
) error {
	if benchmark != nil {
		navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.send_lock.wait", "go.transport",
			"phase", benchmark.Phase, "phaseSequence", benchmark.PhaseSequence,
			"sceneSequence", benchmark.SceneSequence, "messageType", benchmark.MessageType)
	}
	s.mu.Lock()
	...
	if semantic {
		if streamID == "" || kind == "" {
			...
			navtrace.NavigationBenchmarkMessageSent(benchmark, err)
			return err
		}
	...
	if benchmark != nil {
		navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.send_lock.acquired", "go.transport",
			"phase", benchmark.Phase, "phaseSequence", benchmark.PhaseSequence,
			"sceneSequence", benchmark.SceneSequence, "messageType", benchmark.MessageType)
	}
...
func (s *extUiMessageSender) sendSemanticBatchWithBenchmark(
	...
	benchmark *navtrace.NavigationBenchmarkMessage,
) error {
	if benchmark != nil {
		navtrace.NavigationBenchmarkEmit(benchmark.TraceID, "transport.send_lock.wait", "go.transport",
			"phase", benchmark.Phase, "phaseSequence", benchmark.PhaseSequence,
	...
	s.mu.Unlock()
	navtrace.NavigationBenchmarkMessageSent(benchmark, sendErr)
	return sendErr
}
...
func extUiChangedSceneSnapshotMessages(previous, current map[string]any) []map[string]any {
	...
	navtrace.NavigationBenchmarkPrepareSceneMessage(current)
	if previous == nil {
	...
	appendPanelStreams := func(scene map[string]any) {
		if panels, ok := semantic.SemanticScenePanelMaps(scene); ok {
...
func extUiReadMessageWithBenchmark(r io.Reader, timing *navtrace.NavigationBenchmarkReadTiming) (map[string]any, error) {
...
func extUiReadWireMessageWithBenchmark(r io.Reader,
	...
	if timing != nil {
		timing.ReadStartNs = navtrace.NavigationBenchmarkMonotonicNs()
	}
	var hdr [4]byte
	...
	if timing != nil {
		timing.HeaderDoneNs = navtrace.NavigationBenchmarkMonotonicNs()
	}
	...
	if timing != nil {
		timing.PayloadDoneNs = navtrace.NavigationBenchmarkMonotonicNs()
		timing.PayloadBytes = len(payload)
		timing.DecodeStartNs = navtrace.NavigationBenchmarkMonotonicNs()
	}
	...
	if timing != nil {
		timing.DecodeDoneNs = navtrace.NavigationBenchmarkMonotonicNs()
	}
	return msg, nil
}
...
func (r *ExtUiRenderer) ConsumeSemanticSceneExportSuppression() bool {
	...
	if !r.suppressSemanticExport || r.semanticFastPathUnsafe {
		...
		navtrace.NavigationBenchmarkRenderEvent("scene.suppression.rejected",
			"reason", reason, "hadPermit", r.suppressSemanticExport)
		r.suppressSemanticExport = false
		r.deferSemanticRender = false
		return false
	}
	navtrace.NavigationBenchmarkRenderEvent("scene.suppression.accepted")
	r.suppressSemanticExport = false
	r.deferSemanticRender = false
	r.deferSemanticRenderBound = false
...
func (r *ExtUiRenderer) queuePanelActivation(side int, title string,
	...
	if side < 0 || side > 1 {
		navtrace.NavigationBenchmarkUIEvent("panel.activate.direct_rejected",
	...
	deliveredTitle := semantic.String(deliveredShell["title"])
	...
	if deliveredSideOK && side == deliveredSide &&
		...
		navtrace.NavigationBenchmarkUIEvent("panel.activate.direct_accepted",
	...
	if r.send != nil {
		benchmark := navtrace.NavigationBenchmarkPrepareImmediateMessage(patch)
		...
		r.lastCompactScene = semantic.CompactAppSemanticScene(patched)
...
func (r *ExtUiRenderer) SetSemanticMenuState(ctx *vtui.SemanticContext) bool {
	...
	if !supported {
		...
		r.mu.Unlock()
		navtrace.NavigationBenchmarkUIEvent("menu_state.direct_rejected",
	...
	rootSet, rootClear := semantic.SemanticPatchChangedKeys(
		r.lastCompactScene, current, semantic.SemanticMenuStateRootPatchKeys)
	...
	if len(rootSet) == 0 && len(rootClear) == 0 {
		...
		armDirectResult()
		navtrace.NavigationBenchmarkUIEvent("menu_state.direct_accepted",
	...
	benchmark := navtrace.NavigationBenchmarkPrepareImmediateMessage(wire)
	...
	r.lastScene = semantic.SemanticSceneStructuralMapCopy(r.lastScene)
	semantic.ApplyAppScenePatchToSnapshot(r.lastScene, patch)
	if r.lastScene != nil {
		r.lastScene["version"] = extui.SceneVersion
	}
	r.lastCompactScene = semantic.CompactAppSemanticScene(r.lastScene)
	armDirectResult()
	navtrace.NavigationBenchmarkUIEvent("menu_state.direct_accepted",
...
func (r *ExtUiRenderer) QueueSurfaceState(surfaceID string,
	...
	case r.directPanelCatalog != nil:
		rejection = "direct_panel_catalog"
	case surfaceID == "" || !semantic.SemanticEditorSurfaceStateValid(state):
	...
	if rejection == "" && (!surfaceOK || !compactSurfaceOK ||
		semantic.String(surface["id"]) != surfaceID ||
		semantic.String(compactSurface["id"]) != surfaceID ||
		semantic.String(surface["kind"]) != "editor" ||
		semantic.String(compactSurface["kind"]) != "editor") {
		rejection = "surface_identity_mismatch"
	}
	if rejection == "" {
		for _, key := range semantic.SemanticEditorSurfaceIdentityKeys {
			matches := false
			if key == "documentKey" {
				matches = semantic.String(state[key]) == semantic.String(surface[key]) &&
					semantic.String(state[key]) == semantic.String(compactSurface[key])
			} else {
				matches = semantic.Int64(state[key]) == semantic.Int64(surface[key]) &&
					semantic.Int64(state[key]) == semantic.Int64(compactSurface[key])
			}
			if !matches {
	...
	if rejection != "" {
		navtrace.NavigationBenchmarkUIEvent("surface_state.direct_rejected",
			"reason", rejection, "surfaceId", surfaceID,
			"sceneRevision", r.sceneRevision)
		return false
	}

	set, clear := semantic.SemanticPatchChangedKeys(surface, state,
		semantic.SemanticEditorSurfaceStateKeys)
	...
	if set == nil {
		set = make(map[string]any, len(semantic.SemanticEditorSurfaceIdentityKeys))
	}
	for _, key := range semantic.SemanticEditorSurfaceIdentityKeys {
		set[key] = state[key]
	}
	...
	if len(set) == 0 && len(clear) == 0 {
		armDirectResult()
		navtrace.NavigationBenchmarkUIEvent("surface_state.direct_accepted",
	...
	r.lastScene = semantic.SemanticSceneStructuralMapCopy(r.lastScene)
	semantic.ApplyAppScenePatchToSnapshot(r.lastScene, patch)
	r.lastCompactScene = semantic.SemanticSceneStructuralMapCopy(r.lastCompactScene)
	semantic.ApplyAppScenePatchToSnapshot(r.lastCompactScene, patch)
	if r.lastScene != nil {
		r.lastScene["version"] = extui.SceneVersion
	}
...
func semanticPanelCatalogStreamPrefix(panel map[string]any,
	...
	if matches, ok := panel["fastFindMatches"].(map[string]any); ok {
		...
		for _, entry := range entries {
			if entryID := semantic.String(entry["entryId"]); entryID != "" {
...
func (r *ExtUiRenderer) queuePanelCatalogState(side int, panel map[string]any,
	...
	panels, panelsOK := semantic.SemanticScenePanelMaps(r.lastScene)
	...
	baseCatalogRevision := semantic.Int64(panels[side]["catalogRevision"])
	catalogRevision := semantic.Int64(panel["catalogRevision"])
	if catalogRevision <= baseCatalogRevision {
		return false
	}
	entries := semantic.AppMapSlice(panel["entries"])
	...
	if shell, ok := r.lastScene["shell"].(map[string]any); ok {
		shellSet := extui.M{}
		if shellTitle != "" && shellTitle != semantic.String(shell["title"]) {
	...
	sendImmediate := func(message map[string]any) bool {
		benchmark := navtrace.NavigationBenchmarkPrepareImmediateMessage(message)
...
func (r *ExtUiRenderer) CanDeferCoveredTerminalRedraw() bool {
	...
	negotiatedAppSurface := r.nativeCellFrameSuppressed ||
		(r.NativeSemanticSurfaceEnabled &&
			semantic.String(r.lastScene["schema"]) == extui.Schema &&
			semantic.String(r.lastScene["presentation"]) != "text")
...
func semanticSceneOwnsNativeSurface(scene map[string]any) bool {
	if scene == nil || semantic.String(scene["schema"]) != "app" ||
		semantic.String(scene["presentation"]) == "text" {
		return false
	}
...
func semanticContainsFallback(value any) bool {
	...
	case map[string]any:
		kind := semantic.String(typed["kind"])
...
func (r *ExtUiRenderer) queueFullSemanticSceneLocked(scene map[string]any) {
	...
	wire := semantic.SemanticShallowMapCopy(scene)
	if semantic.String(wire["schema"]) == extui.Schema {
		...
		r.lastCompactScene = semantic.CompactAppSemanticScene(wire)
	}
...
func (r *ExtUiRenderer) SetSemanticInputUnchanged() bool {
	...
	if rejection != "" {
		navtrace.NavigationBenchmarkUIEvent("input_unchanged.direct_rejected",
			"reason", rejection, "sceneRevision", r.sceneRevision)
		return false
	}
	r.semanticUpdateTouched = true
	...
	r.panelActivationQueued = false
	r.panelActivationProjected = false
	navtrace.NavigationBenchmarkUIEvent("input_unchanged.direct_accepted",
		"sceneRevision", r.sceneRevision)
	return true
}
...
func (r *ExtUiRenderer) setSemanticSceneIncremental(ctx *vtui.SemanticContext,
	...
	if rejection != "" {
		navtrace.NavigationBenchmarkIncrementalEvent("scene.incremental.rejected",
			"reason", rejection, "sceneRevision", r.sceneRevision)
		r.mu.Unlock()
		return false
	}
	...
	current, supported := Presentation.ProjectScene(ctx)
	if !supported {
		navtrace.NavigationBenchmarkIncrementalEvent("scene.incremental.rejected",
			"reason", "unsupported_projection", "sceneRevision", baseRevision)
		return false
	}
	patch, acknowledgements, valid := current.BuildPatch(previous)
	if !valid {
		navtrace.NavigationBenchmarkIncrementalEvent("scene.incremental.rejected",
			"reason", "invalid_patch", "sceneRevision", baseRevision)
		return false
	}
	empty := semantic.ScenePatchEmpty(patch)
	...
	if empty {
		if direct {
			armDirectResult()
			navtrace.NavigationBenchmarkIncrementalEvent("scene.transition.direct_accepted",
				"result", "unchanged", "sceneRevision", r.sceneRevision)
		}
		return true
	}
	...
	r.lastCompactScene = current.Snapshot()
	r.lastScene = semantic.SemanticSceneStructuralMapCopy(r.lastScene)
	semantic.ApplyAppScenePatchToSnapshot(r.lastScene, patch)
	if r.lastScene != nil {
		r.lastScene["version"] = extui.SceneVersion
...
func (r *ExtUiRenderer) SetSemanticScene(scene map[string]any) {
	scene = semantic.SemanticNormalizeSparsePanelCatalogs(scene)
	benchmark := navtrace.NavigationBenchmarkSceneCompareBegin(scene)
	compareResult := "full_scene"
	if benchmark != nil {
		defer func() {
			navtrace.NavigationBenchmarkSceneCompareEnd(benchmark, compareResult)
		}()
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if r.pendingScene == nil {
		...
		if patch, ok := semanticPanelCatalogPatch(r.lastScene, scene); ok {
			...
			if r.directPanelCatalog != nil {
				...
				if !matches && navtrace.NavigationBenchmarkIsEnabled() {
					navtrace.NavigationBenchmarkRenderEvent("scene.panel_catalog.direct_mismatch",
...
func semanticCoveredTerminalID(previousShell, currentShell map[string]any) (string, bool) {
	for _, shell := range []map[string]any{previousShell, currentShell} {
		if shell == nil || semantic.String(shell["mode"]) != "panels" ||
		...
		if layout, ok := shell["panelLayout"].(map[string]any); ok {
			if semantic.Int(layout["leftBottomInsetRows"]) != 0 ||
				semantic.Int(layout["rightBottomInsetRows"]) != 0 {
				return "", false
			}
		}
	}
	...
	terminalID := semantic.String(previousTerminal["id"])
	if terminalID == "" || terminalID != semantic.String(currentTerminal["id"]) {
		return "", false
	}
...
func semanticPanelCatalogPatch(previous, current map[string]any) (map[string]any, bool) {
	...
	previousPanels, previousOK := semantic.SemanticScenePanelMaps(previous)
	currentPanels, currentOK := semantic.SemanticScenePanelMaps(current)
	...
	diff := semanticPanelCatalogDiff{
		...
		side:              changedSide,
		panelID:           semantic.String(changedPanel["id"]),
		shellID:           semantic.String(currentShell["id"]),
	...
	if commandLine, ok := currentCommandLine.(map[string]any); ok {
		diff.commandLineID = semantic.String(commandLine["id"])
	}
...
func semanticDirectPanelCatalogRemainder(delivered, authoritative map[string]any) (map[string]any, bool) {
	if delivered == nil || authoritative == nil ||
		semantic.String(delivered["type"]) != "panel_catalog" ||
		semantic.String(authoritative["type"]) != "panel_catalog" ||
...
func semanticPanelCatalogTraceRejection(previous, current map[string]any) {
	...
	navtrace.NavigationBenchmarkRenderEvent("scene.panel_catalog.rejected", fields...)
}
...
func semanticPanelCatalogRejection(previous, current map[string]any) (string, string, bool) {
	...
	if semantic.String(previousPanel["kind"]) != "filePanel" ||
		semantic.String(currentPanel["kind"]) != "filePanel" ||
		semantic.String(previousPanel["id"]) == "" ||
		semantic.String(previousPanel["id"]) != semantic.String(currentPanel["id"]) {
	...
	diff := semanticPanelCatalogDiff{
		...
		activeScreen:      ExtUiAnyInt(current["activeScreen"]),
		side:              currentSide,
		panelID:           semantic.String(currentPanel["id"]),
		shellID:           semantic.String(currentShell["id"]),
		commandLine:       currentCommandLine,
		commandLineDiff:   commandLineDiff,
		shellTitle:        currentTitle,
...
func semanticPanelCatalogTransitionSafe(previous, current map[string]any, index int) bool {
	if semantic.String(previous["kind"]) != "filePanel" || semantic.String(current["kind"]) != "filePanel" ||
		semantic.String(previous["id"]) == "" || semantic.String(previous["id"]) != semantic.String(current["id"]) ||
	...
	previousCatalogRevision := semantic.Int64(previous["catalogRevision"])
	currentCatalogRevision := semantic.Int64(current["catalogRevision"])
...
func semanticPanelEntriesEqualExceptSelection(previous, current any) bool {
	previousEntries, previousOK := semantic.SemanticMapSlice(previous)
	currentEntries, currentOK := semantic.SemanticMapSlice(current)
...
func semanticWorkspaceTabsNavigationEquivalent(previous, current any, activeScreen, changedSide int) bool {
	...
	previousTabs, previousOK := semantic.SemanticMapSlice(previousMap["tabs"])
	currentTabs, currentOK := semantic.SemanticMapSlice(currentMap["tabs"])
...
func semanticStreamSnapshot(scene map[string]any,
	...
	case "shell":
		...
		projected := semantic.SemanticShallowMapCopy(shell)
		delete(projected, "commandLine")
		if panels, panelsOK := semantic.SemanticScenePanelMaps(scene); panelsOK {
			compactPanels := make([]map[string]any, 0, len(panels))
			for _, panel := range panels {
				compact := semantic.SemanticShallowMapCopy(panel)
	...
	default:
		if strings.HasPrefix(streamID, "panel/") {
			...
			for side, panel := range panels {
				if semantic.String(panel["id"]) == panelID {
					state["side"] = side
					state["panel"] = panel
					break
				}
...
func semanticSceneWithPanelActivation(scene map[string]any, activeSide int,
	...
	if commandLine != nil {
		...
		if !ok || semantic.String(previousCommandLine["id"]) == "" {
			return nil, "command_line_shape"
		}
		if semantic.String(previousCommandLine["id"]) != semantic.String(commandLine["id"]) {
	...
	out := semantic.SemanticShallowMapCopy(scene)
	...
	strictAliases := semantic.String(scene["schema"]) != extui.Schema
	shellID := semantic.String(shell["id"])
	...
	if legacyValue, present := scene["legacy"]; present {
		...
		if !ok || legacy == nil {
			...
			legacyCopy := semantic.SemanticShallowMapCopy(legacy)
...
func semanticPanelActivationShellSupported(shell map[string]any) bool {
	if semantic.String(shell["mode"]) != "panels" ||
		!ExtUiAnyBool(shell["showPanels"]) ||
		!ExtUiAnyBool(shell["showLeftPanel"]) ||
		!ExtUiAnyBool(shell["showRightPanel"]) ||
		ExtUiAnyBool(shell["wide"]) {
...
func semanticPanelActivationShellCopy(shell map[string]any, previousSide,
	...
	out := semantic.SemanticShallowMapCopy(shell)
...
func semanticPanelActivationPanelsCopy(value any, previousSide,
	...
	patchPanel := func(panel map[string]any, index int) (map[string]any, int, bool) {
		if panel == nil || semantic.String(panel["kind"]) != "filePanel" {
			return nil, 0, false
		}
		side := index
		...
		out := semantic.SemanticShallowMapCopy(panel)
...
func semanticPanelActivationScreensCopy(value any, activeScreen int,
	...
	patchScreen := func(screen map[string]any, index int) (map[string]any, bool) {
		...
		out := semantic.SemanticShallowMapCopy(screen)
...
func semanticSceneComparisonProjection(scene map[string]any) map[string]any {
	if semantic.String(scene["schema"]) == extui.Schema {
...
func semanticSceneWithoutCommandLineRendering(scene map[string]any) (map[string]any, bool) {
	...
	if !ok || commandLine == nil || semantic.String(commandLine["id"]) == "" {
		return nil, false
	}
	rootCopy := semanticSceneComparisonProjection(scene)
	shellCopy := semantic.SemanticShallowMapCopy(shell)
...
func semanticScrubHiddenTerminalPresentationAlias(value any, terminalID string) any {
	switch typed := value.(type) {
	case map[string]any:
		if semantic.String(typed["kind"]) == "terminal" &&
			semantic.String(typed["id"]) == terminalID {
		...
		for _, key := range []string{"shell", "terminal", "frames", "screens"} {
			...
			if !copied {
				...
				out = semantic.SemanticShallowMapCopy(typed)
				copied = true
			}
...
func semanticPanelActivationTraceRejection(previous, current map[string]any) {
	...
	if !previousOK || !currentOK {
		navtrace.NavigationBenchmarkRenderEvent("scene.panel_activation.rejected",
	...
	navtrace.NavigationBenchmarkRenderEvent("scene.panel_activation.rejected", fields...)
}
...
func semanticNonAutocompleteMenus(value any) []map[string]any {
	...
	for _, menu := range menus {
		if semantic.String(menu["role"]) != "autocomplete" {
...
func (r *ExtUiRenderer) Flush() {
	...
	if fallbackRevealReady {
		...
		if r.pendingScene != nil {
			...
			r.lastScene = semantic.SemanticShallowMapCopy(r.pendingScene)
	...
	if !r.fallbackRevealPending && r.pendingPanelActivation != nil {
		...
		r.lastCompactScene = semantic.CompactAppSemanticScene(r.pendingPanelActivationScene)
		r.panelActivationProjected = true
		r.pendingPanelActivation = nil
		r.pendingPanelActivationScene = nil
	}
	if !r.fallbackRevealPending && r.pendingPanelCatalog != nil {
		...
		r.lastCompactScene = semantic.CompactAppSemanticScene(r.pendingPanelCatalogScene)
		r.panelActivationProjected = false
		r.pendingPanelCatalog = nil
		r.pendingPanelCatalogScene = nil
	}
	if !r.fallbackRevealPending && r.pendingCommandLine != nil {
		...
		r.lastCompactScene = semantic.CompactAppSemanticScene(r.pendingCommandLineScene)
		r.panelActivationProjected = false
		r.pendingCommandLine = nil
		r.pendingCommandLineScene = nil
	}
	...
	var benchmarks []*navtrace.NavigationBenchmarkMessage
	if navtrace.NavigationBenchmarkIsEnabled() {
		benchmarks = make([]*navtrace.NavigationBenchmarkMessage, len(messages))
		...
		for i, msg := range messages {
			if navtrace.NavigationBenchmarkString(msg["type"]) == "scene" {
				benchmarks[i] = navtrace.NavigationBenchmarkPrepareRenderMessage(msg)
			}
		}
		for i, msg := range messages {
			if navtrace.NavigationBenchmarkString(msg["type"]) != "scene" {
	...
	for i, msg := range messages {
		var benchmark *navtrace.NavigationBenchmarkMessage
		if i < len(benchmarks) {
...
func RunExternalUI(cols, rows int, execPath string, args []string) error {
	...
	defer semantic.SetPanelCatalogMetadataEnabled(previousPanelCatalogMetadata)
...
func (h *ExtUiHost) readLoop() {
	for {
		var timing *navtrace.NavigationBenchmarkReadTiming
		if navtrace.NavigationBenchmarkIsEnabled() {
			timing = &navtrace.NavigationBenchmarkReadTiming{}
		}
...
func (h *ExtUiHost) handleMessageWithBenchmark(msg map[string]any, timing *navtrace.NavigationBenchmarkReadTiming) {
	switch extUiString(msg, "type") {
	case "focus":
	...
	case "key":
		...
		benchmark := navtrace.NavigationBenchmarkTraceForKey(msg, timing)
		keySequence := 0
	...
	case "ui_action":
		...
		queuedNs := int64(0)
		if benchmark != nil {
			queuedNs = navtrace.NavigationBenchmarkMonotonicNs()
			benchmark.EventAt("ui_task.queued", "go.ipc", queuedNs, "action", benchmark.Action)
		}
		if vtui.FrameManager != nil {
			vtui.FrameManager.PostPriorityTask(func() {
				if semantic.String(action["action"]) == "panel.prepareDrag" {
					response := Presentation.PrepareDrag(action)
				...
				previousBenchmark := navtrace.NavigationBenchmarkSetCurrentUI(benchmark)
				handled := App.RunSemanticAction(action)
				navtrace.NavigationBenchmarkSetCurrentUI(previousBenchmark)
				if benchmark != nil {
...
func (h *ExtUiHost) queuePanelCatalogRows(msg map[string]any) bool {
	...
	vtui.FrameManager.PostTaskWithRedrawDecision(func() bool {
		...
		response, ok := Presentation.CatalogRows(
		...
		if !ok {
			...
			if Presentation.CatalogRowsRetryable(
...
func (h *ExtUiHost) serveLivePanelCatalogMetadata(request map[string]any) {
	...
	response, ok := Presentation.LiveCatalogMetadata(
		extUiString(request, "panelId"), extUiString(request, "path"),
...
func (h *ExtUiHost) servePanelCatalogMetadata(request map[string]any) {
	catalogRevision := extUiInt64(request, "catalogRevision")
	metadataRevision := extUiInt64(request, "metadataRevision")
	response, ok := Presentation.CatalogMetadata(
		extUiString(request, "panelId"), extUiString(request, "path"),
		catalogRevision, metadataRevision,
		ExtUiInt(request, "offset"), ExtUiInt(request, "limit"))
	if !ok {
...
func (h *ExtUiHost) sendEventWithBenchmark(ev *vtinput.InputEvent, benchmark *navtrace.NavigationBenchmarkTrace, keySequence int) {
	if h.reader == nil {
		return
	}
	queue := h.reader.EventChan
	navtrace.NavigationBenchmarkInputQueueBegin(ev, benchmark, keySequence, len(queue), cap(queue))
	select {
	case queue <- ev:
		navtrace.NavigationBenchmarkInputQueueEnd(ev, true, len(queue))
	case <-time.After(500 * time.Millisecond):
		navtrace.NavigationBenchmarkInputQueueEnd(ev, false, len(queue))
...
func externalUIBackendArgs(backend string) []string {
	...
	args := []string{
		"--f4-icon-set=" + string(config.ParseQmlIconSetMode(string(config.App.QmlIconSet))),
		"--f4-font-family=" + Presentation.Options().FontFamily,
		"--f4-font-size=" + strconv.Itoa(config.App.GuiFontSize),
		"--f4-window-geometry-file=" + filepath.Join(
			config.GetF4ConfigDir(), "window-geometry.ini"),
	}
	if branch := Presentation.Options().WorktreeBranch; branch != "" {
...
func (r *ExtUiRenderer) NativeSemanticSurfaceActive() bool {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.NativeSemanticSurfaceEnabled
}
