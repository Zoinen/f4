...
func TestExternalUIBackendArgsScopeQmlIconsToQt(t *testing.T) {
	oldIconSet := config.App.QmlIconSet
	oldFont := config.App.GuiFont
	oldUseSystemFont := config.App.GuiUseSystemMonospace
	oldFontSize := config.App.GuiFontSize
	defer func() {
		config.App.QmlIconSet = oldIconSet
		config.App.GuiFont = oldFont
		config.App.GuiUseSystemMonospace = oldUseSystemFont
		config.App.GuiFontSize = oldFontSize
	}()

	config.App.QmlIconSet = config.QmlIconSetSystem
	config.App.GuiUseSystemMonospace = false
	config.App.GuiFont = "JetBrains Mono"
	config.App.GuiFontSize = 21
	...
	config.App.QmlIconSet = config.QmlIconSetMode("invalid")
...
func TestExtUiRenderer_DirectEditorCursorStateIsTinyAndDefersRender(t *testing.T) {
	...
	if message["type"] != "scene_patch" ||
		semantic.Int64(message["baseRevision"]) != 1 ||
		semantic.Int64(message["revision"]) != 2 {
	...
	if semantic.Int(installed["cursorPos"]) != 1 || len(semantic.AppMapSlice(installed["rows"])) != 500 {
...
func TestExtUiRenderer_UnchangedTaskBoundaryPreservesExistingDirectProofs(t *testing.T) {
	...
	t.Run("catalog", func(t *testing.T) {
		...
		renderer.Flush()
		extUiDrainBufferedMessages(t, &wire)
		panels, ok := semantic.SemanticScenePanelMaps(renderer.lastScene)
		if !ok || len(panels) != 2 {
			t.Fatal("initial scene has no panels")
		}
		panel := semantic.SemanticShallowMapCopy(panels[0])
...
func TestExtUiRenderer_DirectPanelCatalogCanFollowProjectedActivation(t *testing.T) {
	...
	directPanel := semantic.SemanticShallowMapCopy(panels[1])
...
func TestExtUiRenderer_DirectPanelCatalogPrecedesRenderAndReconcilesWithChrome(t *testing.T) {
	...
	changed := semantic.SemanticShallowMapCopy(base)
	changedShell := semantic.SemanticShallowMapCopy(base["shell"].(map[string]any))
	...
	changedCommandLine := semantic.SemanticShallowMapCopy(
...
func TestExtUiRenderer_DirectPanelCatalogCanUpdateInactiveSide(t *testing.T) {
	...
	inactive := semantic.SemanticShallowMapCopy(panels[1])
...
func TestExtUiRenderer_DirectPanelCatalogMismatchForcesAuthoritativeScene(t *testing.T) {
	...
	renderer.Flush()
	extUiDrainBufferedMessages(t, &buf)
	direct := semantic.SemanticShallowMapCopy(basePanel)
	direct["path"] = `D:\new`
	...
	second := semantic.SemanticShallowMapCopy(direct)
	second["path"] = `D:\newer`
	...
	changed["shell"] = semantic.SemanticShallowMapCopy(base["shell"].(map[string]any))
...
func TestExtUiRenderer_DirectPanelCatalogRollbackForcesAuthoritativeScene(t *testing.T) {
	base := rollbackCatalogScene(`D:\Code\f4`, 10, 20, false, "old")
	projected := rollbackCatalogScene(`D:\Code\f4\plugins`, 11, 21, false, "new")
	projectedPanels, ok := semantic.SemanticScenePanelMaps(projected)
...
func TestExtUiRenderer_IncrementalTextPresentationRevealIsAtomic(t *testing.T) {
	...
	renderer.lastScene = semantic.SemanticSceneStructuralMapCopy(renderer.lastScene)
	semantic.ApplyAppScenePatchToSnapshot(renderer.lastScene, patch)
	renderer.lastCompactScene = semantic.CompactAppSemanticScene(renderer.lastScene)
...
func TestExtUiChangedSceneSnapshotMessagesAreStrictlyStreamLocal(t *testing.T) {
	...
	current := semantic.SemanticSceneStructuralMapCopy(previous)
	...
	current = semantic.SemanticSceneStructuralMapCopy(previous)
	currentShell := semantic.SemanticShallowMapCopy(
