...
func registerCountingMediaSource(t *testing.T, broker *ExtUiMediaBroker, filesystem *countingMediaVFS) ImageSourceDescriptor {
	t.Helper()
...
func TestExtUiMediaBrokerCanceledReadDoesNotBreakConcurrentRange(t *testing.T) {
	...
	// Wait for canceled shared flights to release their independent activity reference.
	broker.flightWG.Wait()
	filesystem.mu.Lock()
	opens, closes := filesystem.openCount, filesystem.closeCount
	filesystem.mu.Unlock()
	if opens != 2 || closes != 2 {
...
func TestExtUiMediaBrokerSceneSweepAllowsReusedPanelIDAndLowerRevision(t *testing.T) {
	...
	exportScene := func(panelIDs ...string) { broker.CommitScenePanels(panelIDs) }
