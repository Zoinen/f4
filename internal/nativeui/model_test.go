...
func TestAppMenuFromLegacyDefersClosedMenuBarSubmenus(t *testing.T) {
	...
	closedItems := semantic.AppMapSlice(closed["items"])
	...
	openedItems := semantic.AppMapSlice(opened["items"])
	children := semantic.AppMapSlice(openedItems[0]["items"])
...
func TestBuildAppSceneFromLegacyPromotesShellAndKeepsFallback(t *testing.T) {
	oldPresentation := config.App.GuiPresentation
	...
	defer func() {
		config.App.GuiPresentation = oldPresentation
	...
	if scene["presentation"] != string(config.GuiPresentationGUI) {
		t.Fatalf("unexpected presentation mode: %#v", scene["presentation"])
	}
	if scene["qmlIconSet"] != string(config.QmlIconSetSystem) {
...
func TestAppWorkspaceTabsExposeTypedLucideIcons(t *testing.T) {
	...
	tabs := semantic.AppMapSlice(appQueueAwareWorkspaceTabs(legacy)["tabs"])
	...
	for index := range tabs {
		if got := semantic.String(tabs[index]["surfaceKind"]); got != wantKinds[index] {
			t.Fatalf("tab %d surfaceKind = %q, want %q", index, got, wantKinds[index])
		}
		if got := semantic.String(tabs[index]["iconName"]); got != wantIcons[index] {
			t.Fatalf("tab %d iconName = %q, want %q", index, got, wantIcons[index])
		}
		if got := semantic.String(tabs[index]["text"]); got != wantTitles[index] {
			t.Fatalf("tab %d text = %q, want %q", index, got, wantTitles[index])
		}
		if got := semantic.Int(tabs[index]["number"]); got != index+1 {
			t.Fatalf("tab %d number = %d, want %d", index, got, index+1)
		}
	}
	if semantic.AppBool(tabs[4]["closable"]) {
...
func TestAppWorkspaceTabsPreserveStructuredTitlePrefixes(t *testing.T) {
	...
	if got := semantic.String(tabs[0]["text"]); got != "P projects ─ Documents" {
...
func TestBuildAppSceneExportsTextPresentation(t *testing.T) {
	...
	defer func() { config.App.GuiPresentation = oldPresentation }()

	scene := BuildAppSceneFromLegacy(nil, map[string]any{"type": "scene"})
	if scene["presentation"] != string(config.GuiPresentationText) {
...
func TestAppDetectsMenuBarSubmenuByAuthoritativeGeometry(t *testing.T) {
	...
	if !IsMenuBarSubmenu(submenu, menuBar) {
