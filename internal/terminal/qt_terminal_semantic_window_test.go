...
func TestTerminalSemanticWindowBoundsLargeHistoryAndDoesNotDuplicateRows(t *testing.T) {
	...
	if got := len(semantic.AppMapSlice(mapped["windowRows"])); got != len(model.WindowRows) {
		t.Fatalf("serialized window rows=%d, want %d", got, len(model.WindowRows))
	}
	if !strings.Contains(terminalModelText(semantic.AppMapSlice(mapped["windowRows"])),
		"row-099999 payload") {
...
func TestTerminalSemanticWindowScrollPinsWhileLiveOutputContinues(t *testing.T) {
	...
	if got := semantic.AppInt64(first["viewportStart"]); got != 4321 {
		t.Fatalf("pinned viewport=%d, want 4321", got)
	}
	if semantic.AppBool(first["followTail"]) {
	...
	if got := semantic.AppInt64(second["viewportStart"]); got != 4321 {
		t.Fatalf("live output moved pinned viewport to %d", got)
	}
	if overlap := assertSemanticOverlapStable(t, first, second, "rows"); overlap != len(semantic.AppMapSlice(first["windowRows"])) {
		t.Fatalf("pinned live output rewrote %d of %d stable rows",
			len(semantic.AppMapSlice(first["windowRows"]))-overlap,
			len(semantic.AppMapSlice(first["windowRows"])))
	}
	...
	oldVisibleTop := semantic.Int(second["contentExtent"]) -
		semantic.Int(second["viewportRows"]) - 1
...
func TestTerminalSemanticRowsPublishCompleteParagraphSpan(t *testing.T) {
	...
	if got := semantic.Int(mapped["columns"]); got != 5 {
		t.Fatalf("terminal columns=%d, want 5", got)
	}
	rows := semantic.AppMapSlice(mapped["windowRows"])
	for visualRow := 0; visualRow < 2; visualRow++ {
		var found map[string]any
		for _, row := range rows {
			if semantic.Int(row["visualRow"]) == visualRow {
				found = row
				break
			}
		}
		...
		if start, end := semantic.Int(found["logicalRowStart"]),
			semantic.Int(found["logicalRowEnd"]); start != 0 || end != 2 {
...
func TestTerminalSemanticCurrentScreenKeepsConsoleBottomGravity(t *testing.T) {
	...
	if got := terminalModelText(semantic.AppMapSlice(model.ToMap()["windowRows"])); !strings.HasSuffix(got, "line\n") {
...
func TestTerminalSemanticCommandLineOverlayOmitsIdlePromptAndCursor(t *testing.T) {
	...
	text := terminalModelText(semantic.AppMapSlice(
