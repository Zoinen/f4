...
func TestProjectionSourceRoundTripsGraphemeBoundariesAndInvalidBytes(t *testing.T) {
		...
		for _, width := range []int{1, 2, 4, 7} {
			for _, wrap := range []bool{false, true} {
				t.Run(fmt.Sprintf("%q/%d/%t", source, width, wrap), func(t *testing.T) {
					...
					offsets := []int{0}
					for _, cluster := range VisualClusters(source) {
						offsets = append(offsets, cluster.End)
					}
					// Combining marks have no independent caret cell: round-trip cluster boundaries.
