...
func TestWrapEngine_NoWrapCacheStaysBounded(t *testing.T) {
	...
	for _, layout := range we.fragmentCache {
		if len(layout.fragments) > 1 || layout.pending != nil {
			t.Fatal("unwrapped completed lines must retain only one fragment, not per-cluster content")
		}
	}
...
func TestWrapEngine_NoWrapCacheInvalidatedOnEdit(t *testing.T) {
	...
	we.GetFragments(0)
	we.GetFragments(1)
	cachedBefore := len(we.fragmentCache[0].fragments)
	...
	if len(we.fragmentCache[0].fragments) != 0 {
		t.Error("fragment metadata survived full invalidation")
	}
	if cachedBefore == 0 {
