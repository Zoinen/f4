...
func TestExecuteFileOp_OptimizedRenameConflict(t *testing.T) {
	...
	for {
		...
		case task := <-vtui.FrameManager.TaskChan:
			...
			if top != nil && top.GetTitle() == i18n.Msg("Warning.Title") {
				foundDialog = true
				goto done
			}
		case <-timeout:
...
func TestAskOverwriteUsesWarningPalette(t *testing.T) {
	...
	container := waitForDialog(t, i18n.Msg("Warning.Title"))
	...
	if !testutil.PressKey(dlg, &vtinput.InputEvent{
...
func setDialogCheckbox(t *testing.T, dlg vtui.Container, chkText string, state int) {
	...
	for _, itm := range dlg.GetChildren() {
		if c, ok := itm.(*vtui.Checkbox); ok {
			if testutil.GetCleanText(c) == chkText {
				c.State = state
				return
			}
		}
...
func TestFileOps_UI_RememberOverwrite(t *testing.T) {
	...
	testutil.ClickDialogButton(t, dlg, "Overwrite")
	...
	for {
		...
		case task := <-vtui.FrameManager.TaskChan:
			...
			if vtui.FrameManager.GetTopFrameType() == vtui.TypeDialog {
				...
				if top.GetTitle() == i18n.Msg("Warning.Title") && !top.IsDone() {
...
func TestFileOps_UI_RenameAndAppendUnsupported(t *testing.T) {
	...
	testutil.ClickDialogButton(t, dlg, "Append")
	errDlg := waitForDialog(t, " Unsupported ")
	testutil.ClickDialogButton(t, errDlg, "Ok")

	// 3. Warning should reappear for f1.txt
	dlg2 := waitForDialog(t, i18n.Msg("Warning.Title"))

	// 4. Click Rename
	testutil.ClickDialogButton(t, dlg2, "Rename")
	...
	dlg3 := waitForDialog(t, i18n.Msg("Warning.Title"))

	// 6. Click Overwrite
	testutil.ClickDialogButton(t, dlg3, "Overwrite")
...
func TestFileOps_UI_MoveSkip(t *testing.T) {
	...
	dlg := waitForDialog(t, i18n.Msg("Warning.Title"))
	testutil.ClickDialogButton(t, dlg, "Skip")
...
func TestFileOps_UI_RenameToEmpty(t *testing.T) {
	...
	dlg := waitForDialog(t, i18n.Msg("Warning.Title"))
	testutil.ClickDialogButton(t, dlg, "Rename")

	renDlg := waitForDialog(t, " Rename ")
	// Click Cancel in the Rename dialog
	testutil.ClickDialogButton(t, renDlg, "Cancel")
