...
func ExecuteFileOpWithResult(pf vtui.Frame, srcVfs, dstVfs vfs.VFS, names []string, destInput string, isMove bool, mode int, onResult func(error)) {
	ExecuteFileOpAtWithResult(pf, srcVfs, dstVfs, srcVfs.GetPath(), names, destInput, isMove, mode, onResult)
}
...
func TransferIdentity(filesystem vfs.VFS, itemPath string) (vfs.VFS, string) {
	if provider, ok := filesystem.(interface {
		TransferIdentity(string) (vfs.VFS, string)
	}); ok {
		return provider.TransferIdentity(itemPath)
	}
...
// directory and reports completion on the UI task queue.
func ExecuteFileOpAtWithResult(pf vtui.Frame, srcVfs, dstVfs vfs.VFS, srcBasePath string, names []string, destInput string, isMove bool, mode int, onResult func(error)) {
	executeFileOpAt(pf, srcVfs, dstVfs, srcBasePath, names, destInput, isMove, mode, nil, onResult)
}
