...
// SetOnCancel installs what the Cancel button does. The button itself stays
// private: a caller needs to say what cancelling means, not to reach the
// widget and restyle it.
func (d *FileOpProgressDialog) SetOnCancel(onCancel func()) {
	d.BtnCancel.OnClick = onCancel
}

// ProgressState is what the dialog is showing at one moment.
...
// Progress reports what the user can see. The widgets stay private: a caller
// outside this package wants to read the display, not to redraw it.
func (d *FileOpProgressDialog) Progress() ProgressState {
	return ProgressState{
		CurrentVisible: d.pbCurrent.IsVisible(),
		CurrentPercent: d.pbCurrent.Percent,
		TotalVisible:   d.pbTotal.IsVisible(),
		TotalPercent:   d.pbTotal.Percent,
		Speed:          d.lblSpeed.GetText(),
	}
}

// Cancel presses the Cancel button, and reports whether there was anything to
// press. A dialog whose owner never installed a handler cannot be cancelled,
// which is not the same as cancelling and having nothing happen.
func (d *FileOpProgressDialog) Cancel() bool {
	if d.BtnCancel == nil || d.BtnCancel.OnClick == nil {
		return false
	}
	d.BtnCancel.OnClick()
	return true
