...
type F4HistoryProvider struct {
	...
	pendingKind       string
	pendingID         string
	pendingTrace      *navtrace.NavigationBenchmarkTrace
...
func (hp *F4HistoryProvider) markDirtyLocked(kind, id string, trace *navtrace.NavigationBenchmarkTrace) uint64 {
	hp.revision++
	hp.pendingKind = kind
	hp.pendingID = id
...
func persistHistorySnapshot(snapshot historyPersistenceSnapshot) error {
	...
	err = snapshot.write(snapshot.path, file, 0600)
	if benchmark != nil {
		fields := []any{"kind", snapshot.kind, "historyId", snapshot.id,
...
func (hp *F4HistoryProvider) addFolderHistory(path string, benchmark *navtrace.NavigationBenchmarkTrace) (before, after int, revision uint64) {
	hp.lockForMutation()
...
// PathIdentity supplies the same normalized key as the application path policy.
var PathIdentity = func(path string) (string, bool) { return path, path != "" }
