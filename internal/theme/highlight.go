...
func (fh *FileHighlighter) LoadFromIniAt(ini *ini.File, baseDir string) {
...
func ParseHighlightRules(ini *ini.File) []HighlightRule { return parseHighlightRulesAt(ini, "") }
func parseHighlightRulesAt(ini *ini.File, baseDir string) []HighlightRule {
...
func parseRuleSectionsAt(ini *ini.File, prefix, baseDir string) []ruleSection {
...
func highlightMatchKey(item *vfs.VFSItem, metadataKnown bool) highlightMatchCacheKey {
	return highlightMatchCacheKey{
		...
		ATime:         semantic.SemanticMTimeNanos(item.ATime),
		CTime:         semantic.SemanticMTimeNanos(item.CTime),
