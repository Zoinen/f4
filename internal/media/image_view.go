...
func imageViewBackAttr() uint64 {
	return vtui.Palette[theme.ColViewerText]
}
...
func imageOverlayAttr() uint64 {
	return vtui.Palette[theme.ColViewerStatus]
}
...
// GalleryState identifies where the gallery is: the gallery itself, the
// cursor inside it and the Path under that cursor. A command palette entry
// captures all three when it is built and refuses to run if any moved, which
// is the only thing outside this package needs to know about gallery state.
//
// gal is nil when the gallery is closed.
func (iv *ImageView) GalleryState() (gal *ImageGallery, cursor int, path string) {
	if iv == nil || iv.Gal == nil {
		return nil, 0, ""
	}
	return iv.Gal, iv.Gal.Cursor, iv.GalleryPath()
}

// NewGalleryView builds a view already inside its gallery, with the maps its
// gallery keeps ready. It exists for the command-palette tests, which need a
// gallery position without a directory of real images behind it; nothing in
// the application constructs a view this way.
func NewGalleryView(path string, siblings []string, index, cursor int) *ImageView {
	return &ImageView{
		Path:     path,
		siblings: siblings,
		Index:    index,
...
func (iv *ImageView) overlayLines() []string {
	...
	size := "unknown size"
	if iv.sizeKnown {
		size = numeric.FormatSize(iv.fileSize)
	}
...
func (iv *ImageView) HandleCommand(cmd int, args any) bool {
	...
	if App != nil && App.HandleCommand(cmd, args) {
		return true
	}
