...
func TestHostGridRect(t *testing.T) {
	...
	got := terminal.HostGridRect(win, 790, 600, true)
...
func TestHostTextSizePrefersTheCell(t *testing.T) {
	saveCell := func() func() {
		terminal.HostTextMu.Lock()
		cw, chh, ck := terminal.HostCellW, terminal.HostCellH, terminal.HostCellKnown
		tw, th, tk := terminal.HostTextW, terminal.HostTextH, terminal.HostTextKnown
		terminal.HostTextMu.Unlock()
		return func() {
			terminal.HostTextMu.Lock()
			terminal.HostCellW, terminal.HostCellH, terminal.HostCellKnown = cw, chh, ck
			terminal.HostTextW, terminal.HostTextH, terminal.HostTextKnown = tw, th, tk
			terminal.HostTextMu.Unlock()
		}
	}()
	defer saveCell()
	...
	terminal.HostTextMu.Lock()
	terminal.HostCellW, terminal.HostCellH, terminal.HostCellKnown = 10, 20, true
	terminal.HostTextW, terminal.HostTextH, terminal.HostTextKnown = 999, 999, true
	terminal.HostTextMu.Unlock()
	if w, h, ok := terminal.HostTextSize(80, 25); !ok || w != 800 || h != 500 {
		t.Errorf("with a cell: got %dx%d ok=%v, want 800x500", w, h, ok)
	}

	// Without a cell the reported text area is all there is.
	terminal.HostTextMu.Lock()
	terminal.HostCellKnown = false
	terminal.HostTextMu.Unlock()
	if w, h, ok := terminal.HostTextSize(80, 25); !ok || w != 999 || h != 999 {
		t.Errorf("without a cell: got %dx%d ok=%v", w, h, ok)
	}

	// And without either, nothing is known.
	terminal.HostTextMu.Lock()
	terminal.HostTextKnown = false
	terminal.HostTextMu.Unlock()
	if _, _, ok := terminal.HostTextSize(80, 25); ok {
...
func TestHostPixelsFromIoctlOnAPipe(t *testing.T) {
	...
	if _, _, ok := terminal.HostPixelsFromIoctl(r); ok {
		t.Error("a pipe has no text area")
	}
	if _, _, ok := terminal.HostPixelsFromIoctl(nil); ok {
...
func TestAnswerComplete(t *testing.T) {
	...
	for _, c := range cases {
		if got := terminal.AnswerComplete(c.in, "\x1b[4;"); got != c.want {
...
func TestHostScaleFindsAScaledDisplay(t *testing.T) {
	...
	if got := terminal.HostScale(win, 640, 408); got != 2 {
		t.Errorf("scale: got %d, want 2", got)
	}

	grid := terminal.HostGridRect(win, 640, 408, true)
...
func TestHostScaleLeavesAnUnscaledDisplayAlone(t *testing.T) {
	...
	if got := terminal.HostScale(win, 1280, 816); got != 1 {
		t.Errorf("scale: got %d, want 1", got)
	}
	if got := terminal.HostGridRect(win, 1280, 816, true); got.W != 1280 || got.H != 816 {
...
func TestHostScaleRefusesNonsense(t *testing.T) {
	...
	if got := terminal.HostScale(win, 0, 0); got != 1 {
		t.Errorf("nothing measured: got %d", got)
	}
	if got := terminal.HostScale(win, 9000, 9000); got != 1 {
