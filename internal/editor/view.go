...
func NewEditorViewWith(pt *piecetable.PieceTable, v vfs.VFS, path string, useEditorConfig, buildIndex bool) *EditorView {
	...
	if buf, ok := pt.GetOriginalBuffer().(*AsyncBuffer); ok {
		...
		li.Rebuild(piecetable.New(prefix))
		semanticExtentKnown = len(prefix) == pt.Size()
	} else if buildIndex {
		li.Rebuild(pt)
	...
	ev := &EditorView{
		...
		semanticExtentKnown: semanticExtentKnown,
	}
	if ev.TabSize <= 0 {
		ev.TabSize = 8
	}
...
func (ev *EditorView) DisplayObject(scr *vtui.ScreenBuf) {
	...
	if !ev.EnsureEngineWidth() || semantic.NativeDocumentCellPaintOwned(scr) {
		return
	}
	height := ev.Y2 - ev.Y1
	width := ev.X2 - ev.X1 + 1
...
func (ev *EditorView) editorCursorStateGuard() editorCursorPatchGuard {
	...
	if (config.App.EditorMarkOccurrences && ev.SelActive) || showHorzCross || showVertCross || ev.RectSelActive ||
...
func (guard editorCursorPatchGuard) canPublish(ev *EditorView, handled bool) bool {
	...
	showHorzCross, showVertCross, _, _ := EditorCrossAttrs()
	return !(config.App.EditorMarkOccurrences && ev.SelActive) && !showHorzCross && !showVertCross &&
...
func (ev *EditorView) fillCellsSpan(target []vtui.CharInfo, data []byte, defaultAttr, selAttr uint64, offset int, selActive bool, selMin, selMax int, syntax []uint64, startVisualCol int, isCrossRow bool, crossVCol int, horzCrossAttr, vertCrossAttr uint64, visualRow, tabOrigin, clipLeft, clipRight int, suppliedLinks ...[]viewer.UrlLink) []vtui.CharInfo {
	target = target[:0]
	text := string(data)
	clusters := editorVisualClusters(text)
	var links []viewer.UrlLink
	if len(suppliedLinks) > 0 {
		links = suppliedLinks[0]
	}
	if ev.hoverURL != "" && len(suppliedLinks) == 0 {
		links = ev.urlLinksForLine(offset, offset+len(data))
	}
	...
	for _, cluster := range clusters {
		...
		left, right := max(0, clipLeft-visualCol), min(len(cells), clipRight-visualCol)
		if left < right {
			target = append(target, cells[left:right]...)
		}
		visualCol += w
	}
	return target
...
// chunkReader captures the buffer, file, and mapping used by one scan. It
// reads unchanged original ranges directly from the file and declines edited
// ranges so callers fall back to the piece table.
...
func (ev *EditorView) StartIndexing() {
	...
	if ev.AsyncBuf == nil && ev.Mapped == nil && ev.IndexIsComplete() {
		ev.semanticExtentKnown = true
	...
	// Fully decoded files have a complete line index already. They still need
	// the long-line/NUL safety scan, but no line-index scan.
	...
	maxSize := pt.Size()
	ev.setIndexStatus(IndexStatus{Phase: IndexScanning, Total: int64(maxSize), Scanned: int64(initialOffset), Lines: initialLineCount})
	...
	go func() {
		...
		scannedTo.Store(int64(initialOffset))
		...
		if indexer, ok := filesystem.(vfs.LineIndexer); ok && codepage == 65001 && !edited {
		...
		for absPos < maxSize {
			...
			data := make([]byte, take)
			n, direct := readFromFile(ctx, data, absPos, take)
			var err error
			if direct {
				data = data[:n]
			} else {
				data, err = pt.GetRange(absPos, take)
				if err == piecetable.ErrLoading && buf != nil {
					// Wait for the original range feeding this piece, without polling.
					original, ok := pt.OriginalRange(absPos, take)
					if ok {
						_, err = buf.ReadContext(ctx, original, take)
					}
					if err == nil {
						data, err = pt.GetRange(absPos, take)
					}
...
// Mapping-dependent gestures and pending windows cannot survive an edit.
