...
Loop:
	...
	foundErrors := 0
	for _, itm := range top.GetChildren() {
		if lb, ok := itm.(interface {
			SemanticNode(*vtui.SemanticContext) map[string]any
		}); ok {
			for _, line := range semantic.AppStringSlice(lb.SemanticNode(nil)["items"]) {
...
func TestSession_DiskPersistence(t *testing.T) {
	...
	panel.LastLeftGalleryState = panel.PanelGallerySessionState{
		LayoutMode:  panel.GalleryLayoutColumns,
		ColumnCount: 3,
		Densities: map[panel.GalleryLayoutMode]int{
			panel.GalleryLayoutMasonry: 211,
			panel.GalleryLayoutColumns: 34,
		},
	}
	panel.LastRightGalleryState = panel.PanelGallerySessionState{
		LayoutMode:  panel.GalleryLayoutIcons,
		ColumnCount: 2,
		Densities: map[panel.GalleryLayoutMode]int{
			panel.GalleryLayoutIcons: 144,
		},
	}
	...
	panel.LastLeftGalleryState = panel.DefaultPanelGallerySessionState()
	panel.LastRightGalleryState = panel.DefaultPanelGallerySessionState()
	...
	if panel.LastLeftGalleryState.LayoutMode != panel.GalleryLayoutColumns ||
		panel.LastLeftGalleryState.ColumnCount != 3 ||
		panel.LastLeftGalleryState.Densities[panel.GalleryLayoutMasonry] != 211 ||
		panel.LastLeftGalleryState.Densities[panel.GalleryLayoutColumns] != 34 {
		t.Fatalf("left gallery session state was not persisted: %#v", panel.LastLeftGalleryState)
	}
	if panel.LastRightGalleryState.LayoutMode != panel.GalleryLayoutIcons ||
		panel.LastRightGalleryState.ColumnCount != 2 ||
		panel.LastRightGalleryState.Densities[panel.GalleryLayoutIcons] != 144 {
		t.Fatalf("right gallery session state was not persisted: %#v", panel.LastRightGalleryState)
	}
...
func TestSession_OldFileDefaultsWideOff(t *testing.T) {
	...
	panel.LastWidePanel = 1
	panel.LastLeftGalleryState = panel.PanelGallerySessionState{
		LayoutMode: panel.GalleryLayoutIcons, ColumnCount: 3,
		Densities: map[panel.GalleryLayoutMode]int{panel.GalleryLayoutIcons: 220},
	}
	LoadSession()
	if panel.LastWidePanel != -1 {
	...
	if panel.LastLeftGalleryState.LayoutMode != panel.GalleryLayoutColumns ||
		panel.LastLeftGalleryState.ColumnCount != 2 ||
		len(panel.LastLeftGalleryState.Densities) != 0 {
		t.Fatalf("old session did not migrate Medium to Columns 2: %#v", panel.LastLeftGalleryState)
	}
}
...
func TestActionRename_Selection(t *testing.T) {
	fm := vtui.FrameManager
	fm.Init(vtui.NewSilentScreenBuf())
	theme.SetDefaultF4Palette()
...
func TestNewUTF8EditorPieceTableUsesCompleteProbeDirectly(t *testing.T) {
	...
	pt, asyncBuf := editor.NewUTF8PieceTable(file, int64(len(content)), content, 0)
	if asyncBuf != nil {
...
func TestNewUTF8EditorPieceTableKeepsLargeFilesStreaming(t *testing.T) {
	...
	_, asyncBuf := editor.NewUTF8PieceTable(file, int64(len(content)), content[:editorEncodingProbeSize], 0)
	if asyncBuf == nil {
...
func TestSession_GalleryStatePersistsWithoutPanelPaths(t *testing.T) {
	...
	oldConfig := config.App
	oldLeftPath := panel.LastLeftPath
	oldRightPath := panel.LastRightPath
	oldLeftGalleryState := panel.LastLeftGalleryState
	...
	defer func() {
		...
		panel.LastRightPath = oldRightPath
		panel.LastLeftGalleryState = oldLeftGalleryState
	...
	left := panels.Panels[0].(*panel.FileSystemPanel)
	right := panels.Panels[1].(*panel.FileSystemPanel)
	if !left.SetGalleryLayout(panel.GalleryLayoutGrid, 0) ||
		!left.SetGalleryDensity(panel.GalleryLayoutGrid, 207) ||
		!right.SetGalleryLayout(panel.GalleryLayoutColumns, 3) ||
		!right.SetGalleryDensity(panel.GalleryLayoutColumns, 31) {
	...
	panel.LastRightPath = "/keep/right"
	panel.LastLeftGalleryState = panel.DefaultPanelGallerySessionState()
	panel.LastRightGalleryState = panel.DefaultPanelGallerySessionState()
	panel.LastWorkspaceSessions = nil
	...
	if panel.LastLeftPath != "/keep/left" || panel.LastRightPath != "/keep/right" {
		t.Fatalf("SavePanelPaths=false changed paths: left=%q right=%q",
			panel.LastLeftPath, panel.LastRightPath)
	}
	if len(panel.LastWorkspaceSessions) != 1 {
		t.Fatalf("captured workspace count = %d, want 1", len(panel.LastWorkspaceSessions))
	}
	captured := panel.LastWorkspaceSessions[0]
	if captured.Left.Gallery.LayoutMode != panel.GalleryLayoutGrid ||
		captured.Left.Gallery.Densities[panel.GalleryLayoutGrid] != 207 ||
		captured.Right.Gallery.LayoutMode != panel.GalleryLayoutColumns ||
		captured.Right.Gallery.ColumnCount != 3 ||
		captured.Right.Gallery.Densities[panel.GalleryLayoutColumns] != 31 {
	...
	LoadSession()
	if len(panel.LastWorkspaceSessions) != 1 {
		t.Fatalf("loaded workspace count = %d, want 1", len(panel.LastWorkspaceSessions))
	}
	loaded := panel.LastWorkspaceSessions[0]
	if loaded.Left.Gallery.LayoutMode != panel.GalleryLayoutGrid ||
		loaded.Left.Gallery.Densities[panel.GalleryLayoutGrid] != 207 ||
		loaded.Right.Gallery.LayoutMode != panel.GalleryLayoutColumns ||
		loaded.Right.Gallery.ColumnCount != 3 ||
		loaded.Right.Gallery.Densities[panel.GalleryLayoutColumns] != 31 {
...
func TestActionPanelSettings_FileInfoToggleDoesNotReloadCatalogs(t *testing.T) {
	...
	oldConfig := config.App
	oldUserConfigPath := config.GetUserConfigIniPath
	...
	t.Cleanup(func() {
		config.App = oldConfig
		config.GetUserConfigIniPath = oldUserConfigPath
	})
	...
	leftEntries := append([]*panel.FileEntry(nil), left.Entries...)
	rightEntries := append([]*panel.FileEntry(nil), right.Entries...)
	...
	for _, child := range dlg.GetChildren() {
		...
		case *vtui.Checkbox:
			if control.GetText() == i18n.Msg("PanelSettings.ShowFileInfo") {
				fileInfo = control
			}
		case *vtui.Button:
			if strings.Contains(control.GetText(), i18n.Msg("vtui.Ok")) {
				okButton = control
			}
		}
	}
	...
	if left.IsLoading || right.IsLoading || !reflect.DeepEqual(left.Entries, leftEntries) || !reflect.DeepEqual(right.Entries, rightEntries) {
		t.Fatal("presentation-only setting reloaded catalogs")
	}
...
func (m *mockSlowVFS) Abs(p string) (string, error) { return p, nil }
...
func TestActionAppearanceSettingsSavesQmlIconSet(t *testing.T) {
	...
	config.App.QmlIconSet = config.QmlIconSetLucide
	...
	for _, child := range top.GetChildren() {
		...
		if combo.Menu.Items[0].Text == i18n.Msg("AppearanceSettings.IconSetLucide") &&
			combo.Menu.Items[1].Text == i18n.Msg("AppearanceSettings.IconSetSystem") {
			iconSetCombo = combo
			break
		}
	}
	...
	testutil.ClickDialogButton(t, top, "Ok")
	if config.App.QmlIconSet != config.QmlIconSetSystem {
		t.Fatalf("QML icon set = %q, want system", config.App.QmlIconSet)
	}
}
