...
func TestHistorySearchDrawHighlightsMatchAndSearchTitle(t *testing.T) {
	...
	for x := menu.X1 + 1; x < menu.X2; x++ {
		char := testutil.Rune(scr.GetCell(x, menu.Y1).Char)
		if char == '[' {
			foundBracket = true
		}
		if char == '*' {
	...
	for x := menu.X1 + 1; x < menu.X2; x++ {
		...
		if testutil.Rune(cell.Char) == 'm' && cell.Attributes == vtui.Palette[vtui.ColDialogHighlightText] {
	...
	for x := menu.X1 + 1; x < menu.X2-1; x++ {
		if testutil.Rune(scr.GetCell(x, menu.Y2).Char) == 'F' && testutil.Rune(scr.GetCell(x+1, menu.Y2).Char) == '2' {
			foundF2Hint = true
			break
		}
	}
...
func TestHistorySearchLockColumnAndDetailsAreCapabilityGated(t *testing.T) {
	...
	menu.Show(scr)
	search.draw(scr)
	if got := testutil.Rune(scr.GetCell(menu.X1+1, menu.Y1+1).Char); got != '*' {
		t.Fatalf("lock marker = %q, want *", got)
	}
	if got := testutil.Rune(scr.GetCell(menu.X1+2, menu.Y1+1).Char); got != ' ' {
		t.Fatalf("separator after lock marker = %q, want space", got)
	}
	if got := testutil.Rune(scr.GetCell(menu.X1+3, menu.Y1+1).Char); got != 'e' {
...
func TestHistorySearchResolvesDialogThemeAtRenderTime(t *testing.T) {
	...
	assertTheme := func(selected, box uint64) {
		...
		if got := testutil.Rune(scr.GetCell(menu.X1+1, y).Char); got != '*' {
			t.Fatalf("selected lock marker = %q, want * at the first inner cell", got)
		}
		if got := testutil.Rune(scr.GetCell(menu.X1+2, y).Char); got != ' ' {
			t.Fatalf("separator after lock marker = %q, want space", got)
		}
		if got := scr.GetCell(menu.X1+2, y).Attributes; got != selected {
			t.Fatalf("separator attr = %#x, want selected-row attr %#x", got, selected)
		}
		if got := scr.GetCell(menu.X1+3, y).Attributes; got != selected {
