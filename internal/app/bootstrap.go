...
func Main() {
	...
	defer func() {
		...
		if history, ok := vtui.GlobalHistoryProvider.(*history.F4HistoryProvider); ok {
...
func tryRunDefaultGui() error {
	if portableQtDefault() {
		return gui.RunGui("qt", setupGuiUI)
	}
	if vtui.IsWine() {
...
func SetupUI() {
	...
	history.SamePath = fileops.SameFolderHistoryPath
	...
	InitHelpSystem()
	vtui.FrameManager.EventFilter = applicationActivationFilter(func(e *vtinput.InputEvent) bool { return macroFilter(macro.MacroMgr, e) })
...
func LoadSession() {
	...
	panel.LastLeftGalleryState = panel.LoadUnifiedPanelGallerySessionState(
	...
	panel.LastRightGalleryState = panel.LoadUnifiedPanelGallerySessionState(
