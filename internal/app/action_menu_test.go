...
func TestBuildMenuBarItems_OnClickRunsAction(t *testing.T) {
	...
	clicked := false
	registerMenuTestAction(t, action.Action{
		Name:     "Test.MenuClick",
		Area:     "Editor",
		Label:    "Click me",
		MenuPath: "TestMenu",
...
func TestBuildMenuBarItems_IncludesPluginPanelCommandsInDeclaredMenu(t *testing.T) {
	t.Cleanup(testutil.SetFrameManagerScreens(t, []*vtui.AppScreen{{Frames: []vtui.Frame{&panel.PanelsFrame{CmdLine: cmdline.NewCommandLine("")}}}}, 0))
...
func registerMenuTestAction(t *testing.T, a action.Action) {
	t.Helper()
...
func TestBuildMenuBarItems_TerminalShortcutIsStableAndConditionAware(t *testing.T) {
	oldManager := keymap.GlobalHotkeysMgr
	oldCondition, hadCondition := keymap.ConditionRegistry["menushortcutpreferred"]
	conditionActive := true
	keymap.RegisterCondition("MenuShortcutPreferred", func() bool { return conditionActive })
	defer func() {
		keymap.GlobalHotkeysMgr = oldManager
		if hadCondition {
			keymap.ConditionRegistry["menushortcutpreferred"] = oldCondition
		} else {
			delete(keymap.ConditionRegistry, "menushortcutpreferred")
		}
	}()
