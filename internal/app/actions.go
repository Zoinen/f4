...
func actionFoldersHistory(pf *panel.PanelsFrame) {
	...
	if activePanel := pf.GetActivePanel(); activePanel != nil {
		...
		for historyPos, path := range h {
			if fileops.SameFolderHistoryPath(path, currentPath) {
...
func applyInitialEditorOffset(editor *editor.EditorView, v vfs.VFS) bool {
...
func applyInitialViewerOffset(viewer *viewer.ViewerView) bool {
...
type preparedEditorDocument struct {
	disasmMode int
	mapped     *editor.MappedFile
	binary     bool
	dataOffset int64
	pt         *piecetable.PieceTable
	buf        *editor.AsyncBuffer
...
func snapshotEditorOpeningEncoding() editorOpeningEncoding {
	return editorOpeningEncoding{
		memoryMap:       config.App.EditorMemoryMap,
		defaultCodepage: config.App.EditorDefaultCodePage,
		autodetect:      config.App.EditorAutodetectCodePage,
	}
}
...
func (prepared *preparedEditorDocument) close() {
	...
	if prepared.mapped != nil {
		prepared.mapped.Close()
	}
...
func prepareEditorDocumentWithEncoding(ctx context.Context, v vfs.VFS, path string, f vfs.ReadAtCloser, encoding editorOpeningEncoding) (*preparedEditorDocument, error) {
		...
		if cpID == 65001 {
			if source, ok := v.(interface {
				...
				if data, available := source.immutableBytes(); available && int64(len(data)) == size {
					...
					pt = piecetable.New(data[dataOffset:])
				}
			}
			if pt == nil && encoding.memoryMap {
				var mapErr error
			...
			if pt == nil && mapped != nil {
				pt = piecetable.New(mapped.Bytes())
			}
			if pt == nil {
				pt, buf, err = editor.NewUTF8PieceTableContext(ctx, f, size, header, dataOffset)
				if err != nil {
					return nil, err
				}
			}
			...
			if _, err := fileops.ReadDocumentBytes(ctx, f, fullData[len(header):], int64(len(header))); err != nil {
	...
	navtrace.NavigationBenchmarkUIEvent("editor.open.buffer",
...
func showPreparedEditor(pf *panel.PanelsFrame, v vfs.VFS, path string, prepared *preparedEditorDocument) {
	// A prepared.mapped or lazily loaded file is indexed by StartIndexing below; anything
	// else — an empty buffer, or a file decoded into memory — has its index
	// built with it, as it always has.
	var ev *editor.EditorView
	if prepared.mapped != nil || prepared.buf != nil {
		ev = editor.NewEditorViewIndexedLater(prepared.pt, v, path)
	} else {
		ev = editor.NewEditorView(prepared.pt, v, path)
	}
	ev.Codepage = prepared.codepage
	ev.BinaryFile = prepared.binary
	ev.Utf8BOM = prepared.codepage == 65001 && prepared.dataOffset != 0
	ev.DisasmMode = prepared.disasmMode
	...
	if fileops.GlobalFileState != nil && path != "" && !prepared.binary {
	...
	ev.File = prepared.file
	ev.AsyncBuf = prepared.buf
	semantic.SeedNativeDocumentViewport(ev)
	ev.Mapped = prepared.mapped
	ev.ResizeConsole(pf.LastW, pf.LastH)
	applyInitialEditorOffset(ev, v)
	ev.StartIndexing()
...
type pendingDocumentOpen struct {
	...
	trace  *navtrace.NavigationBenchmarkTrace
}

var PendingDocumentOpens = make(map[*panel.PanelsFrame]*pendingDocumentOpen)

func beginPendingDocumentOpen(pf *panel.PanelsFrame, kind string, v vfs.VFS, path string) *pendingDocumentOpen {
	key := kind + ":" + fileops.FileStateKey(v, path)
	...
	op := &pendingDocumentOpen{key: key, ctx: ctx, cancel: cancel, trace: navtrace.NavigationBenchmarkCurrentUI()}
	PendingDocumentOpens[pf] = op
	return op
}

func CancelPendingDocumentOpen(pf *panel.PanelsFrame) bool {
...
func finishPendingDocumentOpen(pf *panel.PanelsFrame, op *pendingDocumentOpen) bool {
...
func runPendingDocumentOpen(pf *panel.PanelsFrame, v vfs.VFS, op *pendingDocumentOpen, description string, worker func(context.Context) error, complete func(error)) {
	...
	if op.trace != nil {
		...
		worker = func(ctx context.Context) error {
			started := navtrace.NavigationBenchmarkMonotonicNs()
			op.trace.EventAt("document.open.prepare.begin", "go.worker", started)
			err := prepare(ctx)
			finished := navtrace.NavigationBenchmarkMonotonicNs()
		...
		complete = func(err error) {
			previous := navtrace.NavigationBenchmarkSetCurrentUI(op.trace)
			defer navtrace.NavigationBenchmarkSetCurrentUI(previous)
			op.trace.Event("document.open.ui.begin", "go.ui", "queueNs", navtrace.NavigationBenchmarkMonotonicNs()-queuedAt.Load())
			if PendingDocumentOpens[pf] == op && op.ctx.Err() == nil && err == nil && (pf == nil || !pf.IsDone()) {
				navtrace.NavigationBenchmarkPublishScene(op.trace, "document.open.ready")
			}
			publish(err)
...
func ActionPanelSettings(pf *panel.PanelsFrame) {
	...
	btnCancel.OnClick = func() { dlg.Close() }
	btnOk.OnClick = func() {
		reloadPanelCatalogs := config.App.ShowHiddenFiles != (chkHidden.State == 1)
...
func ActionAppearanceSettings(pf *panel.PanelsFrame) {
	...
	iconSetNames := []string{
		i18n.Msg("AppearanceSettings.IconSetLucide"),
		i18n.Msg("AppearanceSettings.IconSetSystem"),
	}
	comboIconSet := vtui.NewComboBox(0, 0, 24, iconSetNames)
	comboIconSet.DropdownOnly = true
	iconSetIndex := 0
	if config.ParseQmlIconSetMode(string(config.App.QmlIconSet)) == config.QmlIconSetSystem {
		iconSetIndex = 1
	}
	comboIconSet.Menu.SetSelectPos(iconSetIndex)
	comboIconSet.Edit.SetText(iconSetNames[iconSetIndex])
	lblIconSet := vtui.NewText(0, 0, i18n.Msg("AppearanceSettings.IconSet"), 0)
	...
	btnOk.OnClick = func() {
		...
		newIconSet := config.QmlIconSetLucide
		if comboIconSet.Menu.SelectPos == 1 {
			newIconSet = config.QmlIconSetSystem
		}
		config.App.QmlIconSet = newIconSet
		...
		if fontChanged {
			vtui.FrameManager.PostTask(func() {
				vtui.ShowMessage(" Appearance ", i18n.Msg("AppearanceSettings.RestartRequired"), []string{"&Ok"})
			})
		}
	}
