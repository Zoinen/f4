...
func TestDocumentReadCompletesShortReadsWithoutPadding(t *testing.T) {
	...
	n, err := fileops.ReadDocumentBytes(context.Background(), file, dst, 0)
	if err != nil || n != len(dst) || !bytes.Equal(dst, file.data) {
		t.Fatalf("read=%q n=%d err=%v", dst, n, err)
	}
	n, err = fileops.ReadDocumentBytes(context.Background(), file, make([]byte, 30), 0)
...
func TestPreparedEditorReusesProbeAndTransfersOwnership(t *testing.T) {
	oldDefault, oldDetect := config.App.EditorDefaultCodePage, config.App.EditorAutodetectCodePage
	defer func() { config.App.EditorDefaultCodePage = oldDefault; config.App.EditorAutodetectCodePage = oldDetect }()
...
func TestPreparedLegacyEditorReadsOnlyProbeRemainder(t *testing.T) {
	oldDefault, oldDetect := config.App.EditorDefaultCodePage, config.App.EditorAutodetectCodePage
	defer func() { config.App.EditorDefaultCodePage = oldDefault; config.App.EditorAutodetectCodePage = oldDetect }()
	config.App.EditorDefaultCodePage = 1251
	config.App.EditorAutodetectCodePage = false
...
func TestEditorPreparationUsesOpeningEncodingSnapshot(t *testing.T) {
	oldDefault, oldDetect := config.App.EditorDefaultCodePage, config.App.EditorAutodetectCodePage
	defer func() { config.App.EditorDefaultCodePage, config.App.EditorAutodetectCodePage = oldDefault, oldDetect }()
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			config.App.EditorDefaultCodePage, config.App.EditorAutodetectCodePage = test.initialDefault, test.initialDetect
			...
			config.App.EditorDefaultCodePage, config.App.EditorAutodetectCodePage = test.laterDefault, false
			release()
			select {
			case got := <-result:
				if got.err != nil {
...
func TestAsyncBufferReadContextSerializesAndCancelsWithoutPolling(t *testing.T) {
	...
	buf := editor.NewAsyncBuffer(context.Background(), file)
	defer buf.Close()
	...
	for _, scenario := range []string{"escape_before_dialog", "cancel_visible_dialog", "supersede_before_dialog"} {
		t.Run(scenario, func(t *testing.T) {
			...
			runPendingDocumentOpen(pf, filesystem, op, "Preparing source", func(ctx context.Context) error {
				var err error
				vv, err = viewer.NewViewerView(ctx, filesystem, "/mock/file.txt")
				return err
			}, func(err error) {
