...
literally. The rules are subtle and, importantly, they are not the same for plain
cursor movement and for selection, so they are written down here.
...
While a selection is being extended, `far2l` deliberately uses a coarser rule:
dividers are treated exactly like spaces, so a selection always covers whole
words.
...
oversight — `far2l` behaves the same way, and code that relies on muscle memory
from Far depends on it.
