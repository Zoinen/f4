...
func (ev *EditorView) StartIndexing() {
	// Hex/decode render by byte offset: the line index is only needed after a
	// switch back to text, and scanning a large binary here is wasted work.
	if ev.HexMode || ev.DecodeMode {
		ev.targetLine = -1
		return
	}
	...
	// Invalidate all work tied to the previous text snapshot before handing
	// one stable piece-table pointer to the new background scan.
	ev.retireEditSession()
	sessionID := ev.editSession
