...
func newUTF8EditorPieceTable(f vfs.ReadAtCloser, size int64, prefix []byte, dataOffset int64) (*piecetable.PieceTable, *AsyncBuffer) {
	...
	if int64(len(logicalPrefix)) == logicalSize {
		return piecetable.New(logicalPrefix), nil
	}
...
func showEditor(pf *PanelsFrame, v vfs.VFS, path string, f vfs.ReadAtCloser) {
	...
	if f != nil {
		...
		if cpID == 65001 {
			...
			if pt == nil && AppConfig.EditorMemoryMap {
				var mapErr error
			...
			if pt == nil {
				pt, buf = newUTF8EditorPieceTable(f, size, header, dataOffset)
			}
		} else {
