...
func TestPanelsFrame_MenuCommands(t *testing.T) {
	...
	sortText := ""
	for _, item := range pf.menuBar.Items[0].SubItems {
		if item.Command == CmLeftSortTime {
			sortText = item.Text
			break
		}
	}
...
func TestPanelsFrame_CtrlViewModes(t *testing.T) {
	...
	for _, tc := range []struct {
		key  uint16
		mode ViewMode
	}{{'1', ViewModeMedium}, {'2', ViewModeBrief}, {'3', ViewModeDetailed}} {
	...
	if pf.widePanel != pf.activeIdx || pf.panels[0].(*FileSystemPanel).viewMode != ViewModeBrief {
