...
func TestDocumentProfilingFixture(t *testing.T) {
	...
	viewer, err := NewViewerView(context.Background(), vfs.NewOSVFS("C:\\Windows"), "C:\\Windows\\pyw.exe")
	if err != nil {
		t.Fatal(err)
	}
	defer viewer.Close()
	...
	applyNativeDocumentViewport(viewer, nativeDocumentGeometry{columns: 310, rows: 49, revision: 1})
	...
	for name, data := range fixtures {
		b.Run(name, func(b *testing.B) {
			b.Run("viewer_open", func(b *testing.B) {
				...
				b.ResetTimer()
				for i := 0; i < b.N; i++ {
					start := navigationBenchmarkMonotonicNs()
					view, err := NewViewerView(context.Background(), vfs.NewOSVFS(dir), path)
					if err != nil {
						b.Fatal(err)
					}
					...
					view.Close()
					samples = append(samples, time.Duration(navigationBenchmarkMonotonicNs()-start))
					drainDocumentBenchmarkTasks()
				}
			...
			for _, wrap := range []bool{false, true} {
				b.Run(fmt.Sprintf("editor_selection_wrap_%t", wrap), func(b *testing.B) {
					vtui.FrameManager.Init(vtui.NewSilentScreenBuf())
					editor := NewEditorView(piecetable.New(data), nil, "fixture.bin")
					defer editor.Close()
