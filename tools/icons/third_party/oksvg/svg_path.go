...
func (svgp *SvgPath) DrawTransformed(r *rasterx.Dasher, opacity float64, t rasterx.Matrix2D) {
	...
	if svgp.fillerColor != nil {
		...
		case rasterx.Gradient:
			...
			rf.SetColor(fillerColor.GetColorFunctionUS(svgp.FillOpacity*opacity, svgp.mAdder.M))
		}
		rf.Draw()
		// default is true
		rf.SetWinding(true)
	...
	if svgp.linerColor != nil {
		...
		strokeScale := matrixStrokeScale(svgp.mAdder.M)
		dash := make([]float64, len(svgp.Dash))
		for i, value := range svgp.Dash {
			dash[i] = value * strokeScale
		}
		r.SetStroke(fixed.Int26_6(svgp.LineWidth*strokeScale*64),
			fixed.Int26_6(svgp.MiterLimit*64), leadLineCap, lineCap,
			lineGap, svgp.LineJoin, dash, svgp.DashOffset*strokeScale)
		svgp.Path.AddTo(&svgp.mAdder)
		...
		case rasterx.Gradient:
			...
			r.SetColor(linerColor.GetColorFunctionUS(svgp.LineOpacity*opacity, svgp.mAdder.M))
		}
		r.Draw()
	}
}

// matrixStrokeScale returns the geometric mean of the transform's X and Y
// scale. SVG strokes participate in the current transform; the upstream
// renderer only transformed path coordinates and left stroke metrics in source
// units, which made strokes stay one pixel wide at every output size.
func matrixStrokeScale(m rasterx.Matrix2D) float64 {
